## Comparison with [1.20.1](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.20.1)
...
<br/>
<table><tr><th></th><th align="left">1.20.1</th><th>23w31a</th></tr><tr><td>DataPack version</td><td><pre>15</pre></td><td><pre>16</pre></td></tr><tr><td>ResourcePack version</td><td><pre>15</pre></td><td><pre>16</pre></td></tr><tr><td>World version</td><td><pre>3465</pre></td><td><pre>3567</pre></td></tr><tr><td>Protocol version</td><td><pre>763</pre></td><td><pre>1073741968</pre></td></tr><tr><td>Use editor</td><td><pre>/</pre></td><td><pre>false</pre></td></tr></table>
<h3>Libraries<a name="version-data-libraries"></a></h3>
<details>
<summary>
Versions
</summary>
<table><tr><th></th><th align="left">1.20.1</th><th>23w31a</th></tr><tr><td>com.mojang:authlib</td><td><pre>4.0.43</pre></td><td><pre>5.0.46</pre></td></tr></table>
</details>
</details>
<hr/>
...
```
Group: wooden_custom_fence -> wooden_fence
```
...
```
Group: wooden_custom_fence_gate -> wooden_fence_gate
```
...
+ commands.random.error.range_too_large: Range of random value must be at most 2147483646
+ commands.random.error.range_too_small: Range of random value must be at least 2
+ commands.random.reset.all.success: Reset %s random sequence(s)
+ commands.random.reset.success: Reset random sequence %s
+ commands.random.roll: %s rolled %s (between %s and %s)
...
+ pack.dropRejected.message: Following entries were not valid packs and were not copied:
 %s
...
+ symlink_warning.title.pack: Added pack(s) contain symbolic links
+ symlink_warning.title.world: World folder contains symbolic links
...
```
XXX.mojang.math.Transformation +1M -4M
```
```
net.minecraft.SharedConstants +1P
```
```
XXX.minecraft.advancements.FrameType +1M -2M | -1P
```
```
XXX.minecraft.network.CompressionDecoder +2M
```
```
XXX.minecraft.network.Connection +26M -6M | +7P -5P
```
```
XXX.minecraft.network.ConnectionProtocol +3M -10M | +2P -5P
```
```
XXX.protocol.game.ClientboundForgetLevelChunkPacket +5M -3M | +1P -2P
```
```
XXX.protocol.handshake.ServerHandshakePacketListener +1M
```
```
XXX.protocol.login.ClientLoginPacketListener +1M
```
```
XXX.protocol.login.ClientboundGameProfilePacket +1M
```
```
XXX.protocol.login.ServerLoginPacketListener +1M | +2P -1P
```
```
XXX.protocol.login.ServerboundHelloPacket +2M -2M | +1P -1P
```
```
XXX.protocol.status.ClientStatusPacketListener +1M
```
```
XXX.minecraft.server.ServerFunctionManager +1M -1M
```
```
XXX.server.commands.ExecuteCommand +23M -21M
```
```
XXX.server.commands.ReturnCommand +2M
```
```
...
```
```
XXX.server.level.ChunkMap +26M -23M | +1P -1P
```
```
XXX.metadata.pack.PackMetadataSection +8M -3M | +2P
```
```
XXX.packs.repository.FolderRepositorySource +2M -5M | +1P
```
```
XXX.packs.repository.ServerPacksSource +4M -3M
```
```
XXX.packs.resources.ResourceMetadata +3M
```
```
XXX.server.players.GameProfileCache$1 +1M -1M
```
```
XXX.server.players.OldUsersConverter$2 +1M -1M
```
```
XXX.server.players.OldUsersConverter$4 +1M -1M
```
```
...
```
```
XXX.minecraft.util.ExtraCodecs +76M -76M | +2P
```
```
XXX.minecraft.util.InclusiveRange +1M
```
```
XXX.datafix.fixes.References +7M | +8P -1P
```
```
XXX.datafix.schemas.V1451_6 +1M
```
```
XXX.datafix.schemas.V1451_6$2 -1M
```
```
XXX.profiling.jfr.JvmProfiler$NoOpProfiler +2M -2M
```
```
XXX.jfr.event.PacketEvent +1M -1M | +1P -1P
```
```
XXX.jfr.event.PacketReceivedEvent +1M -1M
```
```
XXX.util.valueproviders.ConstantFloat -5M
```
```
XXX.minecraft.world.RandomSequence +2M -1M
```
```
XXX.world.effect.MobEffect +4M -4M
```
```
...
```
```
XXX.entity.raid.Raids +4M -1M
```
```
XXX.world.flag.FeatureFlags +1P
```
```
XXX.world.inventory.EnchantmentMenu$3 +1M
```
```
XXX.world.inventory.SmithingMenu +4M -3M
```
```
...
```
```
XXX.world.level.ForcedChunksSavedData +1M
```
```
XXX.level.block.BambooSaplingBlock +1M -1M
```
```
XXX.level.block.BarrierBlock +7M | +1P
```
```
...
```
```
XXX.level.block.CropBlock +2M -1M
```
```
XXX.level.block.DecoratedPotBlock +5M -2M
```
```
...
```
```
XXX.level.block.LiquidBlockContainer +1P -1P
```
```
XXX.level.block.MangrovePropaguleBlock +1M -1M
```
```
XXX.level.block.NetherrackBlock +1M -1M
```
```
XXX.level.block.NyliumBlock +1M -1M
```
```
XXX.level.block.PowderSnowBlock +1M -1M
```
```
XXX.level.block.RootedDirtBlock +1M -1M
```
```
XXX.level.block.SeagrassBlock +2M -2M
```
```
XXX.level.block.SimpleWaterloggedBlock +2M -2M
```
```
XXX.level.block.SlabBlock +1M -1M
```
```
XXX.level.block.StemBlock +1M -1M
```
```
XXX.level.block.SweetBerryBushBlock +1M -1M
```
```
XXX.level.block.TallGrassBlock +1M -1M
```
```
XXX.block.entity.SkullBlockEntity +13M -8M | +1P
```
```
XXX.levelgen.heightproviders.ConstantHeight -5M
```
```
XXX.structure.structures.WoodlandMansionPieces -1M
```
```
XXX.level.pathfinder.Path +5M -3M | +1P -3P
```
```
XXX.saveddata.maps.MapItemSavedData +4M -2M
```
```
XXX.level.storage.LevelStorageSource$LevelStorageAccess +1M
```
```
XXX.level.validation.DirectoryValidator +4M -2M | +1P -1P
```
```
XXX.world.scores.Scoreboard +4M -10M | +1P -8P
```
...
```
XXX.realmsclient.client.RealmsClient +5M -11M | +1P -3P
```
```
XXX.realmsclient.client.RealmsError +1M -4M | +4P -2P
```
```
XXX.realmsclient.dto.PendingInvite +1P -1P
```
```
XXX.realmsclient.dto.RealmsServer +1P -1P
```
```
XXX.realmsclient.gui.ErrorCallback +2M -1M
```
```
XXX.realmsclient.gui.RealmsWorldSlotButton +2P -2P
```
```
XXX.gui.screens.RealmsBackupScreen -2P
```
```
XXX.gui.screens.RealmsBackupScreen$BackupObjectSelectionList -1M
```
```
XXX.gui.screens.RealmsBrokenWorldScreen +1P
```
```
XXX.gui.screens.RealmsConfigureWorldScreen -1M | +4P -5P
```
```
XXX.gui.screens.RealmsInviteScreen -1M
```
```
XXX.gui.screens.RealmsNotificationsScreen +6M -5M | +5P -6P
```
```
XXX.gui.screens.RealmsPlayerScreen -3P
```
```
XXX.gui.screens.RealmsPlayerScreen$InvitedObjectSelectionList -1M
```
```
XXX.gui.components.AbstractSelectionList +1M -3M | -1P
```
```
XXX.gui.components.AbstractSliderButton +2M -2M | +4P -9P
```
```
XXX.gui.components.AbstractWidget +1M -2M | -2P
```
```
XXX.gui.components.ImageWidget +6M -3M | -1P
```
```
XXX.components.events.ContainerEventHandler +2M -2M
```
```
XXX.components.spectator.SpectatorGui +2P -2P
```
```
XXX.components.tabs.TabManager -1M
```
```
XXX.components.toasts.AdvancementToast +1M | +1P
```
```
XXX.components.toasts.SystemToast +2M -1M | +1P
```
```
XXX.components.toasts.Toast -1P
```
```
XXX.components.toasts.TutorialToast +1M | +1P
```
```
XXX.font.providers.TrueTypeGlyphProviderDefinition +2M -4M
```
```
XXX.gui.layouts.FrameLayout +1M
```
```
XXX.gui.layouts.GridLayout +2M
```
```
XXX.gui.layouts.LinearLayout +13M -4M | +2P -2P
```
```
XXX.gui.screens.PauseScreen +1M | +1P -1P
```
```
XXX.gui.screens.Screen +2M -1M
```
```
XXX.gui.screens.SimpleOptionsSubScreen +1M
```
```
XXX.gui.screens.SoundOptionsScreen +1M
```
```
XXX.screens.advancements.AdvancementWidget +1P -1P
```
```
XXX.screens.packs.PackSelectionScreen +5M -2M
```
```
XXX.client.multiplayer.ServerStatusPinger$1 +1M -1M | +1P
```
```
XXX.renderer.debug.BeeDebugRenderer +18M -18M
```
```
XXX.renderer.debug.GameEventListenerRenderer +1M -1M
```
```
XXX.renderer.texture.MissingTextureAtlasSprite +1P -1P
```
```
XXX.renderer.texture.SpriteLoader +9M -8M | +1P
```
```
XXX.renderer.texture.TextureAtlasSprite +2M -2M
```
```
...
```
```
XXX.gametest.framework.TestCommand +9M -6M | +1P
```
```
XXX.minecraft.network.ConnectionProtocol$PacketSet +1M -2M
```
```
XXX.minecraft.network.FriendlyByteBuf +84M -10M | -2P
```
```
XXX.minecraft.network.PacketBundlePacker +1M -1M | +1P -1P
```
```
XXX.minecraft.network.PacketDecoder +1M -1M | +1P -1P
```
```
XXX.minecraft.network.Varint21LengthFieldPrepender +1P -1P
```
```
XXX.network.chat.CommonComponents +1P
```
```
XXX.network.protocol.BundlerInfo$Provider +1P -1P
```
```
XXX.protocol.game.ClientboundLoginPacket +2M -12M | +1P -11P
```
```
XXX.protocol.game.ClientboundRespawnPacket +6M -10M | +1P -9P
```
```
XXX.protocol.game.ClientboundSetDisplayObjectivePacket +2M -2M | +1P -1P
```
```
XXX.protocol.game.DebugPackets +5M -10M
```
```
XXX.minecraft.server.ServerScoreboard +2M -1M
```
```
XXX.server.commands.ChaseCommand +1P
```
```
XXX.server.level.ServerEntity +3P
```
```
XXX.server.network.ServerGamePacketListenerImpl +20M -24M | +2P -6P
```
```
XXX.server.network.ServerLoginPacketListenerImpl +8M -5M | +3P -3P
```
```
XXX.server.network.ServerLoginPacketListenerImpl$State +3P -2P
```
```
XXX.server.packs.FilePackResources +3M -3M | +2P -4P
```
```
XXX.packs.repository.Pack$Info +3M -3M | +2P -1P
```
```
XXX.packs.repository.Pack$ResourcesSupplier +2P -1P
```
```
...
```
```
XXX.world.entity.HumanoidArm +2M -1M | +2P
```
```
...
```
```
XXX.entity.animal.Panda +1M
```
```
XXX.entity.animal.Turtle +1M
```
```
XXX.animal.allay.Allay +1M -2M | -1P
```
```
...
```
```
XXX.entity.monster.Ghast +2M
```
```
XXX.entity.monster.Guardian +1M
```
```
...
```
```
XXX.entity.monster.Phantom +2M -1M
```
```
XXX.entity.monster.Ravager +1M -2M
```
```
XXX.entity.monster.Shulker +1M -2M
```
```
XXX.entity.monster.Strider +1M -2M | -1P
```
```
