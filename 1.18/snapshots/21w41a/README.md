...
# Comparison with <a href="https://github.com/PixiGeko/Minecraft-generated-data/tree/21w40a">21w40a</a>
## Registries
...
```diff
+ ClientLevel level
+ int highPriorityQuota
+ int MAX_HIGH_PRIORITY_QUOTA
+ PriorityQueue toBatchHighPriority
+ Queue toBatchLowPriority
- Level level
...
```diff
+ void <init>(RenderChunkRegion,boolean)
- void <init>(RenderChunkRegion)
```
...
+ Registry CONDITION
+ Registry RULE
+ ResourceKey CONDITION_REGISTRY
+ ResourceKey RULE_REGISTRY
- Registry SURFACE_BUILDER
- ResourceKey CONFIGURED_SURFACE_BUILDER_REGISTRY
- ResourceKey SURFACE_BUILDER_REGISTRY
...
```diff
+ ConfiguredFeature DARK_FOREST_VEGETATION
- ConfiguredFeature DARK_FOREST_VEGETATION_BROWN
- ConfiguredFeature DARK_FOREST_VEGETATION_RED
```
...
```diff
+ RangeDecoratorConfiguration RANGE_BOTTOM_TO_MAX_TERRAIN_HEIGHT
- RangeDecoratorConfiguration RANGE_BOTTOM_TO_120
```
...
+ ResourceKey lambda$codec$0(ResourceLocation)
+ ResourceKey lambda$create$1(String)
+ ResourceKey lambda$elementKey$2(ResourceLocation)
- ResourceKey lambda$create$0(String)
- ResourceKey lambda$elementKey$1(ResourceLocation)
```
...
<details><summary>net.minecraft.server.level.PlayerRespawnLogic</summary>

```diff
+ BlockPos getOverworldRespawnPos(ServerLevel,int,int)
+ BlockPos getSpawnPosInChunk(ChunkPos)
...
```diff
- Registry CONFIGURED_SURFACE_BUILDER
