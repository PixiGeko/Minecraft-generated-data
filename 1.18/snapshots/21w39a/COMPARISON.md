## Comparison with [21w38a](https://github.com/PixiGeko/Minecraft-generated-data/tree/21w38a)
...
<br/>
<table><tr><th></th><th align="left">21w38a</th><th>21w39a</th></tr><tr><td>ResourcePack version</td><td><pre>7</pre></td><td><pre>8</pre></td></tr><tr><td>World version</td><td><pre>2835</pre></td><td><pre>2836</pre></td></tr><tr><td>Protocol version</td><td><pre>1073741866</pre></td><td><pre>1073741867</pre></td></tr></table>
<h3>Libraries<a name="version-data-libraries"></a></h3>
<details>
<summary>
Versions
</summary>
<table><tr><th></th><th align="left">21w38a</th><th>21w39a</th></tr><tr><td>com.mojang:authlib</td><td><pre>3.0.35</pre></td><td><pre>3.1.36</pre></td></tr><tr><td>org.lwjgl:lwjgl-glfw</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-glfw</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-jemalloc</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-jemalloc</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-openal</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-openal</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-opengl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-opengl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-stb</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-stb</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-tinyfd</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-tinyfd</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr></table>
</details>
</details>
<hr/>
...
- selectWorld.pre_worldheight: Incompatible world height
```
...
- XXX.levelgen.placement.EnvironmentScanConfiguration
...
```
XXX.advancements.critereon.LocationPredicate +1M
```
```
XXX.minecraft.core.Direction +1M | +1P
```
```
XXX.server.chase.ChaseServer +1M -1M
```
```
XXX.server.level.ServerLevel +1M -1M
```
```
XXX.server.level.ServerPlayer +3M | +2P
```
```
XXX.server.level.WorldGenRegion +1M -1M
```
```
XXX.datafix.fixes.MissingDimensionFix +1M
```
```
XXX.util.thread.StrictQueue$FixedPriorityQueue -1M | +2P -1P
```
```
XXX.util.valueproviders.IntProviderType +1P
```
```
XXX.level.chunk.ChunkAccess +2M
```
```
XXX.level.chunk.ProtoChunk +4M -4M
```
```
XXX.level.levelgen.Decoratable +1M
```
```
XXX.level.levelgen.NoiseGeneratorSettings +1M -1M | +1P
```
```
XXX.level.levelgen.NoiseSampler +1M -1M | +1P -1P
```
```
...
```
```
XXX.feature.configurations.SimpleBlockConfiguration +5M -5M | -3P
```
```
XXX.levelgen.placement.WaterDepthThresholdDecorator +2M -2M
```
```
XXX.level.storage.DataVersion -2M
```
```
...
```
XXX.realmsclient.client.Ping -1M
```
```
XXX.advancements.critereon.LocationPredicate +1M
```
```
XXX.screens.inventory.EffectRenderingInventoryScreen +5M -5M | -1P
```
```
XXX.inventory.tooltip.ClientBundleTooltip +4M -4M
```
```
XXX.client.sounds.SoundEngine +10M -9M | +2P -1P
```
```
XXX.data.loot.FishingLoot -1M
```
```
XXX.data.worldgen.Features +13M -1M | +5P -2P
```
```
XXX.data.worldgen.Features$Decorators +1P -4P
```
```
XXX.server.level.ChunkMap +5M -6M
```
```
XXX.server.level.PlayerMap +1M -1M
```
```
XXX.server.level.ServerChunkCache +2M -3M
```
```
...
```
```
XXX.ai.goal.RandomLookAroundGoal +1M
```
```
XXX.ai.goal.RangedBowAttackGoal +1M
```
```
XXX.ai.goal.SwellGoal +1M
```
```
XXX.ai.goal.ZombieAttackGoal +1M
```
```
XXX.entity.animal.Fox$SleepGoal +1M
```
```
XXX.entity.animal.Panda$PandaBreedGoal +1M -1M | -1P
```
```
XXX.entity.animal.Rabbit$RabbitJumpControl +1M -1M | -1P
```
```
XXX.entity.monster.Blaze$BlazeAttackGoal +1M
```
```
XXX.entity.monster.Slime$SlimeAttackGoal +1M
```
```
XXX.entity.monster.Vex$VexChargeAttackGoal +1M
```
```
XXX.entity.raid.Raider$HoldGroundAttackGoal +1M
```
```
XXX.world.item.ItemCooldowns$CooldownInstance +1M -1M | -1P
```
```
XXX.level.biome.TerrainShaper +4M -1M | +1P -1P
```
```
XXX.block.state.StateDefinition +4M -3M
```
```
XXX.state.pattern.BlockPatternBuilder +1M
```
```
XXX.levelgen.feature.Feature +3P -3P
```
```
XXX.levelgen.placement.BlockFilterDecorator +2M -2M
```
```
XXX.levelgen.placement.CarvingMaskDecorator -1M
```
```
XXX.levelgen.placement.DecorationContext +1M -1M
```
```
XXX.levelgen.placement.FeatureDecorator +1P -2P
```
