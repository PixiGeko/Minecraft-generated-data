## Comparison with [22w17a](https://github.com/PixiGeko/Minecraft-generated-data/tree/22w17a)
...
<br/>
<table><tr><th></th><th align="left">22w17a</th><th>22w18a</th></tr><tr><td>World version</td><td><pre>3093</pre></td><td><pre>3095</pre></td></tr><tr><td>Protocol version</td><td><pre>1073741906</pre></td><td><pre>1073741907</pre></td></tr></table>
...
```diff
- io.netty:netty-all V4.1.68.Final
+ io.netty:netty-buffer V4.1.77.Final
+ io.netty:netty-codec V4.1.77.Final
+ io.netty:netty-common V4.1.77.Final
+ io.netty:netty-handler V4.1.77.Final
+ io.netty:netty-resolver V4.1.77.Final
+ io.netty:netty-transport V4.1.77.Final
+ io.netty:netty-transport-classes-epoll V4.1.77.Final
+ io.netty:netty-transport-native-epoll V4.1.77.Final
+ io.netty:netty-transport-native-unix-common V4.1.77.Final
...
<table><tr><th></th><th align="left">22w17a</th><th>22w18a</th></tr><tr><td>com.mojang:text2speech</td><td><pre>1.13.9</pre></td><td><pre>1.16.7</pre></td></tr></table>
</details>
</details>
<hr/>
...
<br/>
<table>
<tr><th>Name</th><th>22w17a</th><th>22w18a</th></tr>
<tr><th align="left"><div style="width:290px">multiplayer.disconnect.invalid_public_key_signature</div></th><td>Invalid signature for profile public key.</td><td>Invalid signature for profile public key.

Try restarting your game.</td></tr>
<tr><th align="left"><div style="width:290px">advancements.adventure.avoid_vibration.description</div></th><td>Sneak near a Sculk Sensor, Sculk Shrieker or Warden to prevent it from hearing you</td><td>Sneak near a Sculk Sensor or Warden to prevent it from detecting you</td></tr>
</table>
<br/>
</details>
</details>
...
+ XXX.entity.player.ProfilePublicKey$Trusted
...
+ XXX.loot.functions.SetGoatHornSoundFunction
...
```
net.minecraft.ChatFormatting +1M | +1P
```
```
...
```
```
XXX.minecraft.core.Registry +6P
```
```
XXX.minecraft.data.HashCache$ProviderCache +2M -2M
```
```
XXX.data.advancements.AdvancementProvider -1P
```
```
XXX.data.info.BlockListReport -1M | -1P
```
```
XXX.data.info.RegistryDumpReport -1M | -1P
```
```
XXX.data.loot.LootTableProvider -1P
```
```
XXX.data.recipes.RecipeProvider -1P
```
```
XXX.data.structures.SnbtToNbt$TaskResult +8M -1M | +1P -1P
```
```
XXX.gametest.framework.GameTestHelper$1 +1M -1M
```
```
XXX.minecraft.network.FriendlyByteBuf +15M -13M
```
```
XXX.network.chat.Component$Serializer +1M
```
```
XXX.protocol.login.ServerboundCustomQueryPacket +2M
```
```
XXX.protocol.login.ServerboundKeyPacket +2M -2M
```
```
XXX.network.syncher.EntityDataSerializer$1 +1M -1M | +2P -2P
```
```
XXX.server.commands.EmoteCommands +2M -4M
```
```
XXX.server.dedicated.DedicatedServer +1M -4M | -2P
```
```
XXX.server.dedicated.DedicatedServerProperties +2M | +2P -5P
```
```
XXX.server.network.ServerGamePacketListenerImpl +5M -4M
```
```
XXX.server.network.ServerLoginPacketListenerImpl +1M -1M | +1P -1P
```
```
XXX.server.packs.FilePackResources +1P
```
```
XXX.server.players.PlayerList +14M -10M
```
```
XXX.minecraft.util.StringUtil +1M
```
```
XXX.monster.warden.Warden +1M -2M | +1P
```
```
XXX.entity.player.Player +2M -2M | +1P
```
```
XXX.world.inventory.LoomMenu +3M -1M | +2P
```
```
XXX.world.item.BannerItem +2M
```
```
XXX.world.item.Item +1M -1M
```
```
XXX.item.trading.MerchantOffers +3M
```
```
XXX.world.level.FoliageColor +1M
```
```
XXX.block.entity.BannerPattern +4M -11M | -48P
```
```
XXX.level.gameevent.GameEventDispatcher$1 +1M -1M
```
```
XXX.structure.pools.JigsawPlacement +2M
```
```
XXX.storage.loot.LootTable$Builder +2M -2M
```
```
XXX.loot.entries.AlternativesEntry +2M
```
```
XXX.loot.entries.LootPoolSingletonContainer$Builder +2M -1M
```
```
XXX.loot.functions.SetBannerPatternFunction$Builder +2M -1M
```
...
+ XXX.loot.functions.SetGoatHornSoundFunction$Serializer
- XXX.loot.functions.SetInstrumentFunction
...
```
XXX.gui.screens.RealmsSlotOptionsScreen +7M -4M | +3P -1P
```
```
net.minecraft.ChatFormatting +1M | +1P
```
...
```
```
XXX.minecraft.client.Minecraft +45M -42M
```
```
XXX.minecraft.client.NarratorStatus +1M
```
```
XXX.commands.arguments.MessageArgument +4M
```
```
XXX.network.chat.ChatSender +4M | +1P
```
```
XXX.network.chat.Style +10M | +1P
```
```
XXX.protocol.game.ClientboundCommandSuggestionsPacket +2M -1M
```
```
XXX.protocol.game.ClientboundMapItemDataPacket +3M -1M
```
```
XXX.protocol.game.ClientboundPlayerInfoPacket$PlayerUpdate +2M -1M | +1P
```
```
...
```
```
XXX.world.entity.AnimationState +2M -1M | +2P -1P
```
```
...
```
```
XXX.monster.warden.AngerLevel +1M
```
```
XXX.block.entity.JigsawBlockEntity +2M -3M | +1P -1P
```
```
XXX.block.entity.SculkCatalystBlockEntity +2M -1M
```
