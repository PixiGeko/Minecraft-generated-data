## Comparison with [1.19.3](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.19.3)
...
<br/>
<table><tr><th></th><th align="left">1.19.3</th><th>23w03a</th></tr><tr><td>DataPack version</td><td><pre>10</pre></td><td><pre>11</pre></td></tr><tr><td>World version</td><td><pre>3218</pre></td><td><pre>3320</pre></td></tr><tr><td>Protocol version</td><td><pre>761</pre></td><td><pre>1073741939</pre></td></tr></table>
...
+ argument.time.tick_count_too_low: Tick count must not be lower than %s, found %s
...
+ options.notifications.display_time: Notification Display Time
...
<br/>
<table>
<tr><th>Name</th><th>1.19.3</th><th>23w03a</th></tr>
<tr><th align="left"><div style="width:290px">options.telemetry</div></th><td>Telemetry Data</td><td>Telemetry Data...</td></tr>
</table>
<br/>
</details>
</details>
...
```
net.minecraft.DetectedVersion +1M -1M
```
```
XXX.minecraft.advancements.Advancement +2M
```
```
XXX.commands.arguments.TimeArgument +3M -2M | +2P -1P
```
```
XXX.arguments.coordinates.BlockPosArgument +2M
```
```
XXX.minecraft.core.Vec3i +1M -2M
```
```
XXX.data.structures.StructureUpdater -2M
```
```
XXX.minecraft.locale.Language +1M | +1P -1P
```
```
XXX.minecraft.network.ConnectionProtocol$PacketSet +6M -2M | +2P
```
```
XXX.minecraft.network.PacketListener +1P -1P
```
```
XXX.network.chat.CommonComponents +1M | +2P
```
```
XXX.chat.contents.TranslatableContents +2M -2M | +1P
```
```
XXX.protocol.game.ClientGamePacketListener +2P
```
```
XXX.protocol.game.ClientboundAnimatePacket -1P
```
```
XXX.server.commands.WeatherCommand +1M
```
```
XXX.commands.data.DataCommands +32M -17M | +1P
```
```
XXX.server.level.ServerLevel +4P -8P
```
```
...
```
```
XXX.minecraft.tags.BiomeTags +5P -1P
```
```
XXX.minecraft.tags.ItemTags +1P
```
```
XXX.util.datafix.DataFixTypes +5M
```
```
...
+ XXX.gui.components.GridWidget$RowHelper
+ XXX.gui.components.LayoutSettings
+ XXX.gui.components.LinearLayoutWidget$1
+ XXX.gui.components.LinearLayoutWidget$Orientation
...
```
XXX.blaze3d.systems.RenderSystem -3M
```
```
XXX.mojang.realmsclient.RealmsMainScreen +11M -9M
```
```
XXX.gui.screens.RealmsBackupScreen$BackupObjectSelectionList -1M
```
```
XXX.gui.screens.RealmsDownloadLatestWorldScreen +4P
```
```
XXX.gui.screens.RealmsPlayerScreen$InvitedObjectSelectionList -1M
```
```
XXX.gui.screens.RealmsSelectFileToUploadScreen +3M -2M
```
```
XXX.gui.screens.RealmsSelectFileToUploadScreen$WorldSelectionList -1M
```
```
XXX.gui.screens.RealmsSelectWorldTemplateScreen$WorldTemplateObjectSelectionList -1M
```
```
net.minecraft.DetectedVersion +1M -1M
```
...
```
```
XXX.gui.components.AbstractSelectionList$Entry +2M
```
```
XXX.gui.components.ContainerObjectSelectionList +3M -1M | -1P
```
```
XXX.components.events.ContainerEventHandler +10M -2M
```
```
XXX.components.toasts.AdvancementToast +1P
```
```
...
```
```
XXX.gui.screens.LanguageSelectScreen$LanguageSelectionList$Entry +1M -1M | +2P -1P
```
```
XXX.gui.screens.ReceivingLevelScreen +2M
```
```
XXX.gui.screens.TitleScreen +6M -6M | +1P -3P
```
```
XXX.screens.achievement.StatsScreen$ItemStatisticsList +1M -1M
```
```
XXX.screens.controls.KeyBindsList$CategoryEntry +2M -1M
```
```
XXX.screens.controls.KeyBindsList$Entry +1P
```
```
XXX.screens.controls.KeyBindsScreen +1M
```
```
XXX.screens.multiplayer.ServerSelectionList +4M -5M | +4P -3P
```
```
XXX.screens.multiplayer.ServerSelectionList$OnlineServerEntry +2M
```
```
XXX.screens.packs.PackSelectionModel$EntryBase +1M
```
```
XXX.screens.packs.TransferableSelectionList$PackEntry +5M -1M | -1P
```
```
XXX.screens.recipebook.OverlayRecipeComponent +2M -1M
```
```
XXX.screens.recipebook.RecipeBookComponent +2M -1M
```
```
XXX.screens.telemetry.TelemetryEventWidget$Content +2M -2M | +1P -1P
```
```
XXX.screens.telemetry.TelemetryInfoScreen +4M -3M
```
```
XXX.screens.worldselection.ConfirmExperimentalFeaturesScreen$DetailsScreen$PackList -1M
```
```
XXX.screens.worldselection.EditWorldScreen +10M -10M
```
```
XXX.screens.worldselection.WorldSelectionList -2M
```
```
XXX.client.multiplayer.ClientHandshakePacketListenerImpl +1M -1M
```
```
XXX.client.multiplayer.ServerData +1P
```
```
XXX.client.multiplayer.ServerStatusPinger$1 +1M -1M
```
```
XXX.renderer.entity.ItemRenderer -1M
```
```
XXX.renderer.entity.MagmaCubeRenderer +4M
```
```
XXX.resources.language.LanguageInfo +7M -7M | +1P -1P
```
```
XXX.minecraft.server.PlayerAdvancements +6M -5M | +3P -4P
```
```
XXX.server.commands.BanIpCommands -1P
```
```
XXX.server.commands.CloneCommands +25M -19M | -1P
```
```
XXX.server.commands.FillBiomeCommand -1P
```
```
XXX.minecraft.util.ExtraCodecs +8M -8M | +1P
```
```
XXX.minecraft.util.Mth +4M -26M | -2P
```
```
XXX.datafix.schemas.V1801 +1M -2M
```
```
XXX.datafix.schemas.V2502 +1M -2M
```
```
XXX.datafix.schemas.V2509 +1M -2M
```
```
XXX.datafix.schemas.V2522 +1M -2M
```
```
XXX.datafix.schemas.V2568 +1M -2M
```
```
XXX.datafix.schemas.V2688 +2M -2M
```
```
XXX.datafix.schemas.V2707 -2M
```
```
XXX.datafix.schemas.V3081 +1M -3M
```
```
XXX.datafix.schemas.V3203 +1M
```
```
XXX.datafix.schemas.V501 +1M -2M
```
```
XXX.world.entity.Entity +4M -3M
```
```
XXX.world.entity.LivingEntity +3M -1M | -1P
```
```
...
```
```
XXX.entity.vehicle.Boat +1M -1M
```
```
XXX.entity.vehicle.MinecartHopper -2M | -3P
```
```
XXX.level.biome.Biome +2M -4M
```
```
XXX.level.biome.Biome$BiomeBuilder +1M -1M | +1P -1P
```
```
XXX.level.biome.Biome$Precipitation +1M -3M | -2P
```
```
XXX.state.properties.RotationSegment +1M | +1P
```
```
XXX.level.pathfinder.FlyNodeEvaluator +1M -1M
```
```
XXX.level.pathfinder.NodeEvaluator +1P -1P
```
```
XXX.level.pathfinder.SwimNodeEvaluator +1M -1M
```
```
XXX.level.pathfinder.WalkNodeEvaluator +3M -3M
```
```
XXX.level.storage.LevelSummary +1M
```
```
XXX.level.storage.PrimaryLevelData +1M -1M
```
```
XXX.world.ticks.LevelTicks +4M -3M
```
