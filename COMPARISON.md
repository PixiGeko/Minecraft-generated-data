## Comparison with [23w46a](https://github.com/PixiGeko/Minecraft-generated-data/tree/23w46a)
...
<table><tr><th></th><th align="left">23w46a</th><th>1.20.3-pre1</th></tr><tr><td>DataPack version</td><td><code>25</code></td><td><code>26</code></td></tr><tr><td>ResourcePack version</td><td><code>21</code></td><td><code>22</code></td></tr><tr><td>World version</td><td><code>3691</code></td><td><code>3693</code></td></tr><tr><td>Protocol version</td><td><code>1073741987</code></td><td><code>1073741988</code></td></tr></table>
</details>
...
+ minecraft/blockstates/short_grass.json
- minecraft/models/block/grass.json
+ minecraft/models/block/short_grass.json
- minecraft/models/item/grass.json
+ minecraft/models/item/short_grass.json
- minecraft/textures/block/grass.png
+ minecraft/textures/block/short_grass.png
```
...
```
net.minecraft.Util +4M -3M | +1P -1P
```
```
XXX.data.advancements.AdvancementSubProvider +1M
```
```
XXX.protocol.common.ClientCommonPacketListener +2P -1P
```
```
XXX.server.dedicated.DedicatedServerProperties +1M -3M
```
```
XXX.minecraft.util.HttpUtil +5M -2M | -1P
```
```
XXX.entity.projectile.WindCharge +1M | +1P
```
...
```diff
+ ExecutorService makeIoExecutor()
- ExecutorService makeIoExecutor(String,boolean)
- ExecutorService nonCriticalIoPool()
- ForkJoinWorkerThread lambda$makeExecutor$3(String,AtomicInteger,ForkJoinPool)
+ ForkJoinWorkerThread lambda$makeExecutor$3(String,ForkJoinPool)
+ Thread lambda$makeIoExecutor$4(Runnable)
- Thread lambda$makeIoExecutor$4(String,AtomicInteger,boolean,Runnable)
```
...
```diff
- AdvancementHolder createPlaceholder(String)
```
...
- Optional getServerPackInfo(String,String,String,String,boolean,String)
...
```diff
- boolean checkExistingFile(Path,HashFunction,HashCode)
+ CompletableFuture downloadTo(File,URL,Map,int,ProgressListener,Proxy)
- HashCode downloadAndHash(HashFunction,int,HttpUtil$DownloadProgressListener,InputStream,Path)
- HashCode hashFile(Path,HashFunction)
+ Object lambda$downloadTo$0(ProgressListener,URL,Proxy,Map,File,int)
- Path cachedFilePath(Path,HashCode)
- Path downloadFile(Path,URL,Map,HashFunction,HashCode,int,Proxy,HttpUtil$DownloadProgressListener)
```
...
- net.minecraft.client.resources.server.PackLoadFeedback$Result
...
```
```
XXX.gui.components.EditBox +1M -2M
```
```
XXX.gui.screens.ConnectScreen$1 +1M
```
```
XXX.data.advancements.AdvancementSubProvider +1M
```
...
```diff
- ServerPackManager$PackPromptStatus convertPackStatus(ServerData$ServerPackStatus)
```
