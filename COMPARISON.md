## Comparison with [18w15a](https://github.com/PixiGeko/Minecraft-generated-data/tree/18w15a)

- [Translations](#translations)
- [File structure](#file-structure)

<hr/>
<details><summary>Translations</summary>
<details>
<summary>
Keys
</summary>

```diff
+ biome.minecraft.beaches: Beach
+ biome.minecraft.birch_forest_hills: Birch Forest Hills
+ biome.minecraft.birch_forest: Birch Forest
+ biome.minecraft.cold_beach: Cold Beach
+ biome.minecraft.cold_deep_ocean: Deep Cold Ocean
+ biome.minecraft.cold_ocean: Cold Ocean
+ biome.minecraft.deep_ocean: Deep Ocean
+ biome.minecraft.desert_hills: Desert Hills
+ biome.minecraft.desert: Desert
+ biome.minecraft.extreme_hills_with_trees: Extreme Hills+
+ biome.minecraft.extreme_hills: Extreme Hills
+ biome.minecraft.forest_hills: Forest Hills
+ biome.minecraft.forest: Forest
+ biome.minecraft.frozen_deep_ocean: Deep Frozen Ocean
+ biome.minecraft.frozen_ocean: Frozen Ocean
+ biome.minecraft.frozen_river: Frozen River
+ biome.minecraft.hell: Nether
+ biome.minecraft.ice_flats: Ice Plains
+ biome.minecraft.ice_mountains: Ice Mountains
+ biome.minecraft.jungle_edge: Jungle Edge
+ biome.minecraft.jungle_hills: Jungle Hills
+ biome.minecraft.jungle: Jungle
+ biome.minecraft.lukewarm_deep_ocean: Deep Lukewarm Ocean
+ biome.minecraft.lukewarm_ocean: Lukewarm Ocean
+ biome.minecraft.mesa_clear_rock: Mesa Plateau
+ biome.minecraft.mesa_rock: Mesa Forest Plateau
+ biome.minecraft.mesa: Mesa
+ biome.minecraft.mushroom_island_shore: Mushroom Island Shore
+ biome.minecraft.mushroom_island: Mushroom Island
+ biome.minecraft.mutated_birch_forest_hills: Mutated Birch Forest Hills
+ biome.minecraft.mutated_birch_forest: Mutated Birch Forest
+ biome.minecraft.mutated_desert: Mutated Desert
+ biome.minecraft.mutated_extreme_hills_with_trees: Mutated Extreme Hills+
+ biome.minecraft.mutated_extreme_hills: Mutated Extreme Hills
+ biome.minecraft.mutated_forest: Flower Forest
+ biome.minecraft.mutated_ice_flats: Ice Plains Spikes
+ biome.minecraft.mutated_jungle_edge: Mutated Jungle Edge
+ biome.minecraft.mutated_jungle: Mutated Jungle
+ biome.minecraft.mutated_mesa_clear_rock: Mutated Mesa Plateau
+ biome.minecraft.mutated_mesa_rock: Mutated Mesa Forest Plateau
+ biome.minecraft.mutated_mesa: Mesa (Bryce)
+ biome.minecraft.mutated_plains: Sunflower Plains
+ biome.minecraft.mutated_redwood_taiga_hills: Mutated Redwood Taiga Hills
+ biome.minecraft.mutated_redwood_taiga: Mega Spruce Taiga
+ biome.minecraft.mutated_roofed_forest: Mutated Roofed Forest
+ biome.minecraft.mutated_savanna_rock: Mutated Savanna Plateau
+ biome.minecraft.mutated_savanna: Mutated Savanna
+ biome.minecraft.mutated_swampland: Mutated Swampland
+ biome.minecraft.mutated_taiga_cold: Mutated Cold Taiga
+ biome.minecraft.mutated_taiga: Mutated Taiga
+ biome.minecraft.ocean: Ocean
+ biome.minecraft.plains: Plains
+ biome.minecraft.redwood_taiga_hills: Mega Taiga Hills
+ biome.minecraft.redwood_taiga: Mega Taiga
+ biome.minecraft.river: River
+ biome.minecraft.roofed_forest: Roofed Forest
+ biome.minecraft.savanna_rock: Savanna Plateau
+ biome.minecraft.savanna: Savanna
+ biome.minecraft.sky_island_barren: The End - Barren island
+ biome.minecraft.sky_island_high: The End - High island
+ biome.minecraft.sky_island_low: The End - Floating islands
+ biome.minecraft.sky_island_medium: The End - Medium island
+ biome.minecraft.sky: The End
+ biome.minecraft.smaller_extreme_hills: Extreme Hills Edge
+ biome.minecraft.stone_beach: Stone Beach
+ biome.minecraft.swampland: Swampland
+ biome.minecraft.taiga_cold_hills: Cold Taiga Hills
+ biome.minecraft.taiga_cold: Cold Taiga
+ biome.minecraft.taiga_hills: Taiga Hills
+ biome.minecraft.taiga: Taiga
+ biome.minecraft.void: The Void
+ biome.minecraft.warm_deep_ocean: Warm Deep Ocean
+ biome.minecraft.warm_ocean: Warm Ocean
+ createWorld.customize.buffet.biome: Please select a biome
+ createWorld.customize.buffet.generator: Please select a generator type
+ createWorld.customize.buffet.generatortype: World generator :
+ createWorld.customize.buffet.title: Buffet world customization
+ generator.buffet: Buffet
+ generator.minecraft.nether: Nether
+ generator.minecraft.overworld: Overworld
+ generator.minecraft.the_end: The End
```

</details>
</details>
<details><summary>File structure</summary>
<details>
<summary>
data
</summary>

```diff
+ minecraft/tags/blocks/coral_blocks.json
+ minecraft/tags/blocks/coral_fans.json
- minecraft/tags/blocks/coral_plants.json
- minecraft/tags/blocks/coral.json
+ minecraft/tags/blocks/corals.json
+ minecraft/tags/blocks/dead_coral_blocks.json
+ minecraft/tags/blocks/live_coral_blocks.json
- minecraft/tags/blocks/live_coral.json
+ minecraft/tags/blocks/valid_spawn.json
+ minecraft/tags/items/coral_blocks.json
+ minecraft/tags/items/coral_fans.json
- minecraft/tags/items/coral_plants.json
- minecraft/tags/items/coral.json
+ minecraft/tags/items/corals.json
+ minecraft/tags/items/dead_coral_blocks.json
+ minecraft/tags/items/live_coral_blocks.json
- minecraft/tags/items/live_coral.json
```

</details>
</details>