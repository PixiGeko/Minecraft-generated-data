## Comparison with [25w20a](https://github.com/PixiGeko/Minecraft-generated-data/tree/25w20a)
...
<br/>
<table><tr><th></th><th align="left">25w20a</th><th>25w21a</th></tr><tr><td>DataPack version</td><td><pre>77</pre></td><td><pre>78</pre></td></tr><tr><td>ResourcePack version</td><td><pre>61</pre></td><td><pre>62</pre></td></tr><tr><td>World version</td><td><pre>4428</pre></td><td><pre>4429</pre></td></tr><tr><td>Protocol version</td><td><pre>1073742074</pre></td><td><pre>1073742075</pre></td></tr></table>
</details>
<hr/>
...
<table><tr><th></th><th align="left">25w20a</th><th>25w21a</th></tr><tr><td>minecraft:equippable</td><td><pre>{
  "allowed_entities": [
    "minecraft:llama",
  ...
  "can_be_sheared": true,
  "equip_sound": "minecraft:entity.llama.swag",
  "shearing_sound": "minecraft:item.llama_carpet.unequip",
  "slot": "body"
}</pre></td></tr></table>
<br/>
</details>
...
<table><tr><th></th><th align="left">25w20a</th><th>25w21a</th></tr><tr><td>minecraft:equippable</td><td><pre>{
  ...
  "equip_on_interact": true,
  "equip_sound": "minecraft:entity.horse.saddle",
  "shearing_sound": "minecraft:item.saddle.unequip",
  "slot": "saddle"
}</pre></td></tr></table>
<br/>
</details>
...
<br/>
<table>
<tr><th>Name</th><th>25w20a</th><th>25w21a</th></tr>
<tr><th align="left"><div style="width:290px">menu.custom_screen_info.contents</div></th><td>The contents of this screen are controlled by third-party servers and maps that are not owned, operated, or supervised by Mojang Studios or Microsoft.
...
If this screen prevents you from playing, you can also disconnect from server by using button below.</td><td>The contents of this screen are controlled by third-party servers and maps that are not owned, operated, or supervised by Mojang Studios or Microsoft.
...
If this screen prevents you from playing, you can also disconnect from the current server by using the button below.</td></tr>
<tr><th align="left"><div style="width:290px">music.game.mice_on_venus</div></th><td>C418 - Mice On Venus</td><td>C418 - Mice on Venus</td></tr>
<tr><th align="left"><div style="width:290px">music.game.oxygene</div></th><td>C418 - Oxygene</td><td>C418 - Oxygène</td></tr>
<tr><th align="left"><div style="width:290px">music.game.swamp.labyrinthine</div></th><td>Lena Raine - Labryinthine</td><td>Lena Raine - Labyrinthine</td></tr>
</table>
<br/>
</details>
</details>
...
```
XXX.minecraft.commands.Commands +1M | +1P
```
```
XXX.protocol.common.ServerCommonPacketListener +1P
```
```
XXX.server.commands.DebugConfigCommand +6M -2M
```
```
XXX.dialog.input.TextInput +5M -1M | +2P
```
...
```diff
- CompletableFuture lambda$register$3(CommandContext,SuggestionsBuilder)
+ Component lambda$config$3(GameProfile)
- Component lambda$config$5(GameProfile)
- int lambda$register$4(CommandContext)
- int showDialog(CommandSourceStack,UUID,Holder)
- ServerConfigurationPacketListenerImpl findConfigPlayer(MinecraftServer,UUID)
...
```diff
- DataResult lambda$static$2(TextInput)
- int maxLength()
- Optional multiline()
- String lambda$static$1()
- void <init>(int,Component,boolean,String,int,Optional)
+ void <init>(int,Component,boolean,String)
```
...
```
```
XXX.screens.configuration.RealmsConfigureWorldScreen +3M | +1P
```
```
XXX.util.task.SwitchMinigameTask +1P -1P
```
```
XXX.minecraft.client.Options +1M
```
```
XXX.client.animation.AnimationDefinition +1M
```
```
XXX.components.toasts.NowPlayingToast +2M | +2P
```
```
XXX.font.glyphs.BakedGlyph +2M -3M
```
```
XXX.render.state.GuiRenderState +2M -1M
```
```
XXX.render.state.GuiRenderState$Node +1M | +1P
```
```
XXX.gui.screens.Screen +4M -3M
```
```
XXX.screens.achievement.StatsScreen -1M
```
```
XXX.screens.dialog.ButtonListDialogScreen +5M -5M | +1P -1P
```
```
XXX.screens.dialog.DialogScreens$Factory +1P -1P
```
```
XXX.screens.dialog.MultiActionInputFormDialogScreen +6M -6M
```
```
XXX.screens.dialog.ServerLinksDialogScreen +3M -5M | -1P
```
```
XXX.screens.dialog.SimpleInputFormDialogScreen +7M -7M
```
```
XXX.dialog.submit.SubmitMethodHandlers$SendCustom +1M -1M
```
```
XXX.screens.inventory.BookViewScreen +1M -1M
```
```
XXX.client.model.BatModel +2P
```
```
XXX.client.model.BreezeModel +6P
```
```
XXX.client.multiplayer.ClientCommonPacketListenerImpl +2M | +1P
```
```
XXX.client.multiplayer.ClientConfigurationPacketListenerImpl +1M
```
```
XXX.renderer.chunk.CompileTaskDynamicQueue +1P
```
```
XXX.renderer.chunk.SectionCompiler$Results -1P
```
```
XXX.client.sounds.MusicManager +1M | +1P
```
```
XXX.minecraft.commands.Commands +1M | +1P
```
...
```diff
+ boolean shouldPlayButtonBeActive(RealmsServer)
```
...
```diff
- RealmsConfigureWorldScreen getNewScreenWithKnownData(RealmsServer)
- void <init>(RealmsMainScreen,long,RealmsServer,PreferredRegionsDto)
...
```diff
- float getFinalSoundSourceVolume(SoundSource)
```
...
```diff
- KeyframeAnimation bake(ModelPart)
```
...
- void renderChar(BakedGlyph$GlyphInstance,Matrix4f,VertexConsumer,int,boolean)
+ void renderChar(BakedGlyph$GlyphInstance,Matrix4f,VertexConsumer,int)
- void renderEffect(BakedGlyph$Effect,Matrix4f,VertexConsumer,int,boolean)
+ void renderEffect(BakedGlyph$Effect,Matrix4f,VertexConsumer,int)
```
...
```diff
- void submitBlitToCurrentLayer(BlitRenderState)
- void submitGlyphToCurrentLayer(GuiElementRenderState)
+ void submitGuiElementToCurrentLayer(GuiElementRenderState)
```
...
```diff
- void submitGlyph(GuiElementRenderState)
```
...
```diff
+ boolean defaultHandleClickEvent(ClickEvent,Minecraft,Screen)
+ boolean handleClickEvent(ClickEvent)
- void clickCommandAction(LocalPlayer,String,Screen)
+ void clickCommandAction(Minecraft,String,boolean)
- void defaultHandleClickEvent(ClickEvent,Minecraft,Screen)
- void defaultHandleGameClickEvent(ClickEvent,Minecraft,Screen)
- void handleClickEvent(Minecraft,ClickEvent)
```
...
```diff
- void <init>(Screen,ButtonListDialog,DialogConnectionAccess)
+ void <init>(Screen,ButtonListDialog)
- void populateBodyElements(LinearLayout,ButtonListDialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,ButtonListDialog)
- void populateBodyElements(LinearLayout,Dialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,Dialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,ButtonListDialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,ButtonListDialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog)
```

</details>
<details>
<summary>
net.minecraft.client.gui.screens.dialog.MultiActionInputFormDialogScreen
</summary>

```diff
- Button lambda$populateBodyElements$0(InputFormControlSet,DialogConnectionAccess,InputFormDialog$SubmitAction)
+ Button lambda$populateBodyElements$0(InputFormControlSet,InputFormDialog$SubmitAction)
- void <init>(Screen,MultiActionInputFormDialog,DialogConnectionAccess)
+ void <init>(Screen,MultiActionInputFormDialog)
- void populateBodyElements(LinearLayout,Dialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,Dialog)
- void populateBodyElements(LinearLayout,MultiActionInputFormDialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,MultiActionInputFormDialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,MultiActionInputFormDialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,MultiActionInputFormDialog)
```
...
```diff
- Stream createListActions(ButtonListDialog,DialogConnectionAccess)
+ Stream createListActions(ButtonListDialog)
- Stream createListActions(ServerLinksDialog,DialogConnectionAccess)
+ Stream createListActions(ServerLinksDialog)
- void <init>(Screen,ServerLinksDialog,DialogConnectionAccess)
+ void <init>(Screen,ServerLinksDialog)
+ void dialogInit(Dialog)
+ void dialogInit(ServerLinksDialog)
```

</details>
<details>
<summary>
net.minecraft.client.gui.screens.dialog.SimpleInputFormDialogScreen
</summary>

```diff
- void <init>(Screen,SimpleInputFormDialog,DialogConnectionAccess)
+ void <init>(Screen,SimpleInputFormDialog)
- void dialogInit(Dialog,DialogConnectionAccess)
+ void dialogInit(Dialog)
- void dialogInit(SimpleInputFormDialog,DialogConnectionAccess)
+ void dialogInit(SimpleInputFormDialog)
- void populateBodyElements(LinearLayout,Dialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,Dialog)
- void populateBodyElements(LinearLayout,SimpleInputFormDialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,SimpleInputFormDialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog)
- void updateHeaderAndFooter(HeaderAndFooterLayout,SimpleInputFormDialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,SimpleInputFormDialog)
```

</details>
<details>
<summary>
net.minecraft.client.gui.screens.dialog.submit.SubmitMethodHandlers$SendCustom
</summary>

```diff
+ void lambda$createCallback$0(CustomSubmitMethod,ClientPacketListener,Map)
- void lambda$createCallback$0(CustomSubmitMethod,DialogConnectionAccess,Map,DialogScreen)
```
...
```diff
- DialogConnectionAccess createDialogAccess()
```
...
```diff
- void showNowPlayingToastIfNeeded()
```
