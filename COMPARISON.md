## Comparison with [1.19](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.19)
...
<table><tr><th></th><th align="left">1.19</th><th>22w24a</th></tr><tr><td>World version</td><td><code>3105</code></td><td><code>3106</code></td></tr><tr><td>Protocol version</td><td><code>759</code></td><td><code>1073741916</code></td></tr></table>
</details>
...
+ gui.abuseReport.reason.alcohol_tobacco_drugs.description: Someone is talking about or otherwise promoting drugs or alcohol.
...
+ gui.abuseReport.reason.defamation_impersonation_false_information.description: Someone is damaging someone else's reputation, pretending to be someone they're not, or sharing false information.
...
+ gui.abuseReport.reason.harassment_or_bullying.description: Someone is shaming, attacking, or bullying you or someone else. This includes when someone is repeatedly trying to contact you or someone else without consent.
...
+ gui.abuseReport.reason.non_consensual_intimate_imagery: Non-consentual intimate imagery
...
+ gui.abuseReport.reason.self_harm_or_suicide.description: Someone is threatening to harm themselves in real life, or talking about harming yourself in real life.
+ gui.abuseReport.reason.terrorism_or_violent_extremism: Terrorism or violent extremism
+ gui.abuseReport.reason.terrorism_or_violent_extremism.description: Someone is talking about, promoting, or threatening with acts of terrorism or violent extremism for political, religious, ideological, or other reasons.
+ gui.abuseReport.reason.title: Select Report Category
+ gui.abuseReport.send.error_message: An error was returned while sending your report: '%s'
...
+ gui.chatReport.report_sent_msg: Thank you for reporting inappropriate behavior.
...
+ gui.chatReport.selected_chat: %s Chat Messages Selected to Report
...
+ gui.chatReport.title: Report Chat
...
+ gui.socialInteractions.tooltip.report: Create report for %s
...
```
```
XXX.animal.allay.Allay +13M -4M | +17P -3P
```
```
...
```diff
- void assertEntitiesPresent(EntityType,BlockPos,int,double)
```
...
```diff
- PlayerChatMessage removeUnsignedContent()
```
...
- boolean isDancing()
- boolean isDuplicationItem(ItemStack)
+ boolean isFlying()
- boolean isSpinning()
+ boolean shouldListen(ServerLevel,GameEventListener,BlockPos,GameEvent,GameEvent$Context)
- boolean shouldStopDancing()
- float getSpinningProgress(float)
+ TagKey getListenableEvents()
- void defineSynchedData()
- void duplicateAllay()
+ void onSignalReceive(ServerLevel,GameEventListener,BlockPos,GameEvent,Entity,Entity,float)
- void removeInteractionItem(Player,ItemStack)
- void resetDuplicationCooldown()
- void setDancing(boolean)
- void setJukeboxPlaying(BlockPos,boolean)
- void updateDuplicationCooldown()
```
...
```diff
- com.mojang.realmsclient.util.ExponentialBackoff$Task
...
- net.minecraft.client.multiplayer.chat.LoggedChat$Player
- net.minecraft.client.multiplayer.chat.LoggedChat$WithId
...
```
XXX.minecraft.client.Minecraft +3M | +1P
```
```
XXX.client.gui.Gui -1M
```
```
XXX.client.gui.GuiComponent +2M
```
```
XXX.gui.screens.PauseScreen +1M
```
```
XXX.screens.multiplayer.WarningScreen +1M
```
```
XXX.screens.social.PlayerEntry +2M -1M | +3P
```
```
XXX.client.multiplayer.ClientPacketListener +2M
```
```
XXX.network.chat.ChatType$Narration +2M -2M
```
```
XXX.network.chat.ChatType$TextDisplay +2M -2M
```
...
- void setCurrentServer(RealmsServer,String)
- void updateReportEnvironment(ReportEnvironment)
```
...
```diff
- void <init>(Component,Component,Component)
```
...
```diff
- void lambda$new$0(Minecraft,ReportingContext,UUID,Button)
+ void lambda$new$0(PlayerSocialManager,UUID,String,Button)
- void lambda$new$2(PlayerSocialManager,UUID,String,Button)
```
...
```diff
- GameProfile getSenderProfile(ChatSender)
- UUID guessChatUUID(Component)
```
...
```diff
+ ChatType$Narration decorated(ChatDecoration,ChatType$Narration$Priority)
- ChatType$Narration decorated(ChatTypeDecoration,ChatType$Narration$Priority)
+ Component lambda$decorate$1(Component,ChatSender,ChatDecoration)
- Component lambda$decorate$1(Component,ChatSender,ChatTypeDecoration)
```
...
```diff
+ ChatType$TextDisplay decorated(ChatDecoration)
- ChatType$TextDisplay decorated(ChatTypeDecoration)
+ Component lambda$decorate$1(Component,ChatSender,ChatDecoration)
- Component lambda$decorate$1(Component,ChatSender,ChatTypeDecoration)
```
