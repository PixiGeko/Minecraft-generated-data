## Comparison with [25w21a](https://github.com/PixiGeko/Minecraft-generated-data/tree/25w21a)
...
<br/>
<table><tr><th></th><th align="left">25w21a</th><th>1.21.6-pre1</th></tr><tr><td>DataPack version</td><td><pre>78</pre></td><td><pre>79</pre></td></tr><tr><td>ResourcePack version</td><td><pre>62</pre></td><td><pre>63</pre></td></tr><tr><td>World version</td><td><pre>4429</pre></td><td><pre>4430</pre></td></tr><tr><td>Protocol version</td><td><pre>1073742075</pre></td><td><pre>1073742076</pre></td></tr></table>
</details>
<hr/>
...
```diff
+ universal_tags/dialog_action_type.json
...
```diff
+ minecraft/dialog/quick_actions.json
...
+ XXX.dialog.submit.CommandTemplate
+ XXX.dialog.submit.CustomSubmitMethod
+ XXX.dialog.submit.ParsedTemplate
+ XXX.dialog.submit.SubmitMethodTypes
...
```
```
XXX.network.codec.ByteBufCodecs$17 +4M -4M | +2P -1P
```
```
XXX.network.codec.ByteBufCodecs$19 +2M -2M
```
```
XXX.network.codec.ByteBufCodecs$20 +2M -2M
```
```
XXX.network.codec.ByteBufCodecs$22 +3M -3M | -1P
```
```
XXX.network.codec.ByteBufCodecs$24 +3M -4M | +1P -2P
```
```
XXX.network.codec.ByteBufCodecs$26 +5M -3M | +2P -3P
```
```
XXX.network.codec.ByteBufCodecs$28 +3M -4M | +2P -2P
```
```
XXX.network.codec.ByteBufCodecs$31 +3M -5M | +3P -4P
```
```
XXX.network.codec.ByteBufCodecs$33 +2M -2M
```
```
XXX.network.codec.ByteBufCodecs$35 +5M | +2P -1P
```
```
XXX.server.dialog.ButtonListDialog +1M | +1P
```
```
XXX.server.dialog.DialogListDialog +1M -1M | +1P -1P
```
```
XXX.server.dialog.Dialogs +2P
```
```
XXX.server.dialog.MultiActionDialog +1M -1M | +1P -1P
```
```
XXX.server.network.ServerCommonPacketListenerImpl +1M -1M
```
```
XXX.minecraft.tags.DialogTags +1P
```
```
XXX.minecraft.util.CommonColors +1P
```
```
XXX.datafix.fixes.MapIdFix +1M
```
...
```diff
- boolean isPathPartPortable(String)
```
...
```diff
+ void <init>(GameTestHelper,Level,BlockPos,float,GameProfile,GameType)
- void <init>(GameTestHelper,Level,GameProfile,GameType)
```
...
+ Object decode(ByteBuf)
+ void <init>(int,BiFunction,StreamCodec)
...
<details>
<summary>
net.minecraft.network.codec.ByteBufCodecs$33
</summary>
...
<details>
<summary>
net.minecraft.network.codec.ByteBufCodecs$35
</summary>

```diff
- JsonElement decode(ByteBuf)
- Object decode(Object)
- void <init>(int)
- void encode(ByteBuf,JsonElement)
...
<details>
<summary>
net.minecraft.server.dialog.ButtonListDialog
</summary>
...
```diff
- void send(Packet,ChannelFutureListener)
...
- XXX.renderer.special.PlayerHeadSpecialRenderer$PlayerHeadRenderInfo
...
```
XXX.blaze3d.opengl.GlRenderPass +1M -1M | +1P -1P
```
```
net.minecraft.FileUtil +1M
```
...
```
```
XXX.client.gui.Font +1M
```
```
XXX.gui.components.AbstractWidget +1M
```
```
XXX.gui.components.MultiLineLabel$1 +2M
```
```
XXX.gui.screens.ConfirmLinkScreen +2M -2M | +1P
```
```
XXX.screens.dialog.ButtonListDialogScreen +7M -7M | +1P -2P
```
```
XXX.screens.dialog.DialogScreen$WarningScreen +1M -1M
```
```
XXX.screens.dialog.ServerLinksDialogScreen +2M -2M
```
```
XXX.dialog.body.DialogBodyHandlers +2M -1M
```
```
XXX.dialog.body.DialogBodyHandlers$PlainMessageHandler +3M -2M
```
```
XXX.screens.inventory.BookViewScreen +1M -1M
```
```
XXX.client.multiplayer.ClientCommonPacketListenerImpl +2M -1M
```
```
XXX.client.multiplayer.ClientConfigurationPacketListenerImpl$2 +1M -1M
```
```
XXX.client.renderer.RenderPipelines -1P
```
```
XXX.renderer.blockentity.ShulkerBoxRenderer +2M
```
```
XXX.renderer.blockentity.SignRenderer +1M
```
```
XXX.renderer.blockentity.SkullBlockRenderer +2M -1M
```
```
XXX.renderer.chunk.SectionRenderDispatcher$RenderSection -2M | -1P
```
```
XXX.renderer.item.SpecialModelWrapper -1M | -1P
```
```
XXX.renderer.special.BannerSpecialRenderer +1M
```
```
XXX.renderer.special.BedSpecialRenderer +1M
```
```
XXX.renderer.special.ChestSpecialRenderer +1M
```
```
XXX.renderer.special.ConduitSpecialRenderer +1M
```
```
XXX.renderer.special.DecoratedPotSpecialRenderer +1M
```
```
XXX.renderer.special.HangingSignSpecialRenderer +1M
```
```
XXX.protocol.common.ServerboundCustomClickActionPacket +1M | +1P
```
```
XXX.server.dialog.ConfirmationDialog +3M -3M | +2P -2P
```
```
XXX.server.dialog.NoticeDialog +2M -2M | +2P -2P
```
```
XXX.entity.decoration.ItemFrame +1M
```
```
XXX.entity.player.Inventory +2P
```
```
...
```diff
- void drawMultipleIndexed(Collection,GpuBuffer,VertexFormat$IndexType,Collection,Object)
+ void drawMultipleIndexed(Collection,GpuBuffer,VertexFormat$IndexType,Collection)
```
...
```diff
- List splitIgnoringLanguage(FormattedText,int)
```
...
```diff
- boolean areCoordinatesInRectangle(double,double)
```
...
```diff
- Style getStyleAtCentered(int,int,int,double,double)
- Style getStyleAtLeftAligned(int,int,int,double,double)
```
...
```diff
+ Button lambda$populateBodyElements$0(ClickAction)
- Button lambda$populateBodyElements$0(DialogControlSet,ActionButton)
+ ClickAction createCancelAction(ButtonListDialog)
+ void <clinit>()
- void lambda$updateHeaderAndFooter$1(HeaderAndFooterLayout,DialogControlSet,ActionButton)
- void lambda$updateHeaderAndFooter$2(HeaderAndFooterLayout)
+ void populateBodyElements(LinearLayout,ButtonListDialog,DialogConnectionAccess)
+ void populateBodyElements(LinearLayout,Dialog,DialogConnectionAccess)
- void populateBodyElements(LinearLayout,DialogControlSet,ButtonListDialog,DialogConnectionAccess)
- void populateBodyElements(LinearLayout,DialogControlSet,Dialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,ButtonListDialog,DialogConnectionAccess)
+ void updateHeaderAndFooter(HeaderAndFooterLayout,Dialog,DialogConnectionAccess)
- void updateHeaderAndFooter(HeaderAndFooterLayout,DialogControlSet,ButtonListDialog,DialogConnectionAccess)
- void updateHeaderAndFooter(HeaderAndFooterLayout,DialogControlSet,Dialog,DialogConnectionAccess)
```

</details>
<details>
<summary>
net.minecraft.client.gui.screens.dialog.DialogScreen$WarningScreen
</summary>

```diff
+ void lambda$new$0(Minecraft,DialogConnectionAccess,MutableObject,boolean)
- void lambda$new$0(Minecraft,MutableObject,boolean)
```
...
```diff
- ActionButton createDialogClickAction(ServerLinksDialog,ServerLinks$Entry)
- ActionButton lambda$createListActions$0(ServerLinksDialog,ServerLinks$Entry)
+ ClickAction createDialogClickAction(ServerLinksDialog,ServerLinks$Entry)
+ ClickAction lambda$createListActions$0(ServerLinksDialog,ServerLinks$Entry)
```
...
```diff
- LayoutElement createBodyElement(DialogScreen,DialogBody)
+ LayoutElement createBodyElement(Screen,DialogBody)
- void runActionOnParent(DialogScreen,Style)
```
...
```diff
- LayoutElement createControls(DialogScreen,DialogBody)
- LayoutElement createControls(DialogScreen,PlainMessage)
+ LayoutElement createControls(Screen,DialogBody)
+ LayoutElement createControls(Screen,PlainMessage)
- void lambda$createControls$0(DialogScreen,Style)
```
...
```diff
- void showDialog(Holder,DialogConnectionAccess,Screen)
- void showDialog(Holder,Screen)
+ void showDialog(Holder)
```
...
```diff
- void getExtents(Direction,float,Set)
- void prepareModel(PoseStack,Direction,float)
```
...
```diff
- void applyInHandTransforms(PoseStack)
```
...
+ RenderType getRenderType(SkullBlock$Type,ResolvableProfile,ResourceLocation)
...
```diff
+ int getDynamicTransformIndex()
+ void setDynamicTransformIndex(int)
```
...
+ ClickAction noButton()
+ ClickAction yesButton()
- void <init>(CommonDialogData,ActionButton,ActionButton)
+ void <init>(CommonDialogData,ClickAction,ClickAction)
```
...
- ActionButton action()
+ ClickAction button()
- void <init>(CommonDialogData,ActionButton)
+ void <init>(CommonDialogData,ClickAction)
```
...
- void fillFromWorld(Level,BlockPos,Vec3i,boolean,List)
```
