## Comparison with [1.15-pre1](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.15-pre1)
...
<table><tr><th></th><th align="left">1.15-pre1</th><th>1.15-pre2</th></tr><tr><td>World version</td><td><code>2218</code></td><td><code>2219</code></td></tr><tr><td>Protocol version</td><td><code>565</code></td><td><code>566</code></td></tr></table>
</details>
...
```
```
XXX.level.block.BedBlock +2M
```
```
XXX.level.block.ChestBlock +7M -4M | +3P -3P
```
```
XXX.level.block.ChestBlock$2 +6M -4M
```
```
XXX.level.block.ChestBlock$3 +5M -3M | +1P
```
...
```diff
- Direction getConnectedDirection(BlockState)
- DoubleBlockCombiner$BlockType getBlockType(BlockState)
```
...
- boolean lambda$combine$0(LevelAccessor,BlockPos)
+ Container getContainer(BlockState,Level,BlockPos,boolean)
- Container getContainer(ChestBlock,BlockState,Level,BlockPos,boolean)
- DoubleBlockCombiner$BlockType getBlockType(BlockState)
- DoubleBlockCombiner$Combiner opennessCombiner(LidBlockEntity)
- DoubleBlockCombiner$NeighborCombineResult combine(ChestBlock,BlockState,Level,BlockPos,boolean)
+ float getCombinedOpenness(LidBlockEntity,BlockState,Level,BlockPos,float)
+ Object combineWithNeigbour(BlockState,LevelAccessor,BlockPos,boolean,ChestBlock$ChestSearchCallback)
...
```diff
+ MenuProvider acceptDouble(ChestBlockEntity,ChestBlockEntity)
+ MenuProvider acceptSingle(ChestBlockEntity)
+ Object acceptDouble(ChestBlockEntity,ChestBlockEntity)
- Object acceptDouble(Object,Object)
- Object acceptNone()
+ Object acceptSingle(ChestBlockEntity)
- Object acceptSingle(Object)
- Optional acceptDouble(ChestBlockEntity,ChestBlockEntity)
- Optional acceptNone()
- Optional acceptSingle(ChestBlockEntity)
```
...
```diff
- Float2FloatFunction acceptNone()
+ Object acceptDouble(ChestBlockEntity,ChestBlockEntity)
- Object acceptDouble(Object,Object)
- Object acceptNone()
+ Object acceptSingle(ChestBlockEntity)
...
```
XXX.screens.worldselection.CreateWorldScreen +1M
```
```
XXX.client.model.GuardianModel +1M
```
```
XXX.client.player.LocalPlayer +1M
```
```
XXX.renderer.blockentity.BedRenderer +1M
```
```
XXX.level.block.GrassPathBlock +1M
```
```
XXX.level.block.HoneyBlock +7M -3M
```
...
```diff
- boolean lambda$render$0(LevelAccessor,BlockPos)
```
...
```diff
- boolean doesEntityDoHoneyBlockSlideEffects(Entity)
+ void doLandingParticleEffect(Level,BlockPos,Entity)
+ void doParticleEffects(Entity,Level,BlockPos,int,double,double,double,double,double,double)
+ void doSlideDownParticleEffects(Level,BlockPos,Entity)
- void doSlideMovement(Entity)
- void maybeDoSlideAchievement(Entity,BlockPos)
- void maybeDoSlideEffects(Level,Entity)
- void showJumpParticles(Entity)
- void showParticles(Entity,int)
- void showSlideParticles(Entity)
```
