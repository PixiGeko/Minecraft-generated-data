## Comparison with [1.18-pre7](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.18-pre7)
...
<table><tr><th></th><th align="left">1.18-pre7</th><th>1.18-pre8</th></tr><tr><td>World version</td><td><code>2854</code></td><td><code>2855</code></td></tr><tr><td>Protocol version</td><td><code>1073741879</code></td><td><code>1073741880</code></td></tr></table>
</details>
...
```
XXX.server.level.ServerChunkCache +1P
```
```
XXX.minecraft.util.BitStorage +1P
```
```
XXX.minecraft.util.CrudeIncrementalIntIdentityHashBiMap +2M
```
```
XXX.datafix.fixes.ChunkToProtochunkFix +14M -8M
```
```
XXX.ai.goal.PathfindToRaidGoal +2P
```
```
XXX.entity.monster.Ravager +1M
```
```
XXX.item.crafting.RecipeManager +5M -6M | +1P
```
```
XXX.level.chunk.GlobalPalette +1M
```
```
XXX.level.chunk.PalettedContainer +2M
```
...
```diff
- CrudeIncrementalIntIdentityHashBiMap copy()
- void <init>(Object[],int[],Object[],int,int)
```
...
- Dynamic fixChunkData(Dynamic)
- Dynamic lambda$makeRule$0(Dynamic)
+ Dynamic lambda$makeRule$4(Dynamic,ShortList)
- Dynamic lambda$repackBiomes$1(Dynamic,ByteBuffer)
- Dynamic lambda$repackBiomes$2(Dynamic,Dynamic)
- Dynamic lambda$repackTicks$5(Dynamic,int)
- Dynamic lambda$repackTicks$6(Dynamic,ShortList)
- Dynamic lambda$repackTicks$7(Dynamic,Stream)
- Dynamic repackBiomes(Dynamic)
- Dynamic repackTicks(Dynamic)
+ IllegalStateException lambda$makeRule$5()
+ Optional lambda$makeRule$0(Typed)
+ Optional lambda$makeRule$1(Dynamic)
+ ShortArrayList lambda$makeRule$2(int)
- ShortArrayList lambda$repackTicks$3(int)
+ Typed lambda$makeRule$6(OpticFinder,Type,Typed)
...
+ void lambda$makeRule$3(List,Dynamic)
- void lambda$repackTicks$4(List,Dynamic)
```
...
```diff
- boolean lambda$registerGoals$1(LivingEntity)
```
...
```diff
- JsonSyntaxException lambda$fromJson$8(String)
...
+ Recipe lambda$byKey$6(ResourceLocation,Map)
- Stream lambda$getRecipeIds$7(Map)
+ Stream lambda$getRecipeIds$8(Map)
- Stream lambda$getRecipes$6(Map)
+ Stream lambda$getRecipes$7(Map)
- void lambda$replaceRecipes$10(Map,ImmutableMap$Builder,Recipe)
+ void lambda$replaceRecipes$11(Map,Recipe)
```
...
- void <init>(IdMap,PalettedContainer$Strategy,PalettedContainer$Data)
```
...
```
XXX.minecraft.client.Minecraft +1M -1M | +1P -1P
```
```
XXX.renderer.chunk.ChunkRenderDispatcher$RenderChunk$ResortTransparencyTask +2M -1M
```
...
```diff
+ void delayCrash(CrashReport)
- void delayCrash(Supplier)
```
...
```diff
+ ChunkRenderDispatcher$ChunkTaskResult lambda$doTask$1(ChunkRenderDispatcher$ChunkTaskResult,Throwable)
- ChunkRenderDispatcher$ChunkTaskResult lambda$doTask$2(ChunkRenderDispatcher$ChunkTaskResult,Throwable)
