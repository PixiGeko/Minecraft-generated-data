## Comparison with [1.20.5-pre1](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.20.5-pre1)
...
<table><tr><th></th><th align="left">1.20.5-pre1</th><th>1.20.5-pre2</th></tr><tr><td>DataPack version</td><td><code>39</code></td><td><code>40</code></td></tr><tr><td>World version</td><td><code>3829</code></td><td><code>3830</code></td></tr><tr><td>Protocol version</td><td><code>1073742009</code></td><td><code>1073742010</code></td></tr></table>
</details>
...
```
advancements.adventure.brush_armadillo.title: Isn't iIt Scute?
advancements.husbandry.remove_wolf_armor.title: Snip it!hear Brilliance
arguments.item.predicate.unknown: Unknown item predicate '%s''
mco.configure.world.buttons.switchminigame: Switch mMinigame
mco.minigame.world.changeButton: Select aAnother mMinigame
mco.minigame.world.restore: Ending mMinigame...
mco.minigame.world.selected: Selected mMinigame:
mco.minigame.world.slot.screen.title: Switching wWorld...
mco.minigame.world.starting.screen.title: Starting mMinigame...
mco.minigame.world.stopButton: End mMinigame
mco.minigame.world.switch.title: Switch mMinigame
mco.minigame.world.title: Switch rRealm to mMinigame
```
...
```
XXX.commands.arguments.MessageArgument$Message +5M -2M
```
```
XXX.core.particles.DustColorTransitionOptions +2M -4M | +1P
```
```
XXX.minecraft.network.Connection +4M -1M
```
...
- int hashCode()
+ MessageArgument$Part[] getParts()
- MessageArgument$Part[] parts()
+ String getText()
- String text()
- String toString()
```
...
+ Float lambda$static$2(DustColorTransitionOptions)
+ Float lambda$static$6(DustColorTransitionOptions)
- Vector3f lambda$static$3(DustColorTransitionOptions)
+ Vector3f lambda$static$5(DustColorTransitionOptions)
```
...
```diff
- ChannelInboundHandler createFrameDecoder(BandwidthDebugMonitor,boolean)
- ChannelOutboundHandler createFrameEncoder(boolean)
+ void configureSerialization(ChannelPipeline,PacketFlow,BandwidthDebugMonitor)
- void configureSerialization(ChannelPipeline,PacketFlow,boolean,BandwidthDebugMonitor)
- void syncAfterConfigurationChange(ChannelFuture)
```
...
```
```
XXX.font.providers.FreeTypeUtil +2M -1M | +2P
```
```
XXX.minecraft.network.PacketListener +1M -1M
```
```
XXX.network.protocol.PacketUtils +1M
```
```
XXX.protocol.game.ServerboundSetCreativeModeSlotPacket +2M -1M | +1P -1P
```
```
XXX.packs.repository.PackRepository +3M -1M
```
```
XXX.minecraft.sounds.SoundEvents +1P -1P
```
```
XXX.datafix.fixes.EntityZombieSplitFix +6M -2M | +1P
```
```
XXX.datafix.fixes.ItemStackComponentizationFix +8M -9M
```
```
XXX.datafix.schemas.V702 +2M -2M
```
```
...
```
```
XXX.item.crafting.RecipeManager +7M -12M | +1P -1P
```
```
...
```
```
XXX.level.storage.LevelStorageSource +4M -2M
```
...
```diff
- boolean copyFromFont(FT_Face,int)
+ void copyFromFont(FT_Face,int)
```
...
```diff
- boolean checkError(int,String)
- void assertError(int,String)
+ void checkError(int,String)
```
...
```diff
- ReportedException makeReportedException(Exception,Packet,PacketListener)
```
...
```diff
- String displayPackList(Collection)
- String lambda$displayPackList$0(Pack)
...
```diff
- Dynamic lambda$changeSchemaToZombieVillager$2(int,Dynamic)
- Dynamic lambda$fix$1(Dynamic)
- Pair fix(String,Typed)
+ Pair getNewNameAndTag(String,Dynamic)
- Type lambda$new$0()
- Typed changeSchemaToZombieVillager(Typed,int)
...
```diff
- boolean lambda$isValidPlayerName$20(int)
+ boolean lambda$isValidPlayerName$21(int)
+ Dynamic lambda$fixFireworkExplosion$20(Dynamic)
- Dynamic lambda$fixProfileProperties$24(OptionalDynamic,Map$Entry,Pair)
+ Dynamic lambda$fixProfileProperties$25(OptionalDynamic,Map$Entry,Pair)
- Dynamic lambda$makeRule$26(ItemStackComponentizationFix$ItemStackData)
- Dynamic lambda$makeRule$27(Dynamic)
+ Dynamic lambda$makeRule$27(ItemStackComponentizationFix$ItemStackData)
+ Dynamic lambda$makeRule$28(Dynamic)
- List lambda$fixProfileProperties$23(Dynamic)
+ List lambda$fixProfileProperties$24(Dynamic)
- Pair lambda$fixProfileProperties$22(Dynamic)
+ Pair lambda$fixProfileProperties$23(Dynamic)
- Stream lambda$fixProfileProperties$25(OptionalDynamic,Map$Entry)
+ Stream lambda$fixProfileProperties$26(OptionalDynamic,Map$Entry)
- String lambda$fixProfileProperties$21(Dynamic)
+ String lambda$fixProfileProperties$22(Dynamic)
```
...
- TypeTemplate lambda$registerEntities$1(Schema)
...
```diff
- float[] collectCandidateStepUpHeights(AABB,List,float)
...
```diff
- boolean canFallAtLeast(double,double,float)
+ boolean isAboveGround()
- boolean isAboveGround(float)
```
...
```diff
- boolean lambda$getRecipeFor$0(Container,Level,RecipeHolder)
- boolean lambda$getRecipeFor$1(Container,Level,RecipeHolder)
+ boolean lambda$getRecipeFor$2(Container,Level,RecipeHolder)
+ boolean lambda$getRecipeFor$3(Container,Level,Map$Entry)
- boolean lambda$getRecipesFor$2(Container,Level,RecipeHolder)
+ boolean lambda$getRecipesFor$5(Container,Level,RecipeHolder)
- Collection byType(RecipeType)
- Collection getOrderedRecipes()
+ ImmutableMap$Builder lambda$apply$0(RecipeType)
+ Map byType(RecipeType)
+ Map lambda$apply$1(Map$Entry)
+ Map lambda$replaceRecipes$9(RecipeType)
+ Pair lambda$getRecipeFor$4(Map$Entry)
...
+ String lambda$getRecipesFor$6(Level,RecipeHolder)
+ void lambda$replaceRecipes$10(Map,ImmutableMap$Builder,RecipeHolder)
```
...
```diff
- Dynamic lambda$readLevelDataTagFixed$5(DataFixer,int,Dynamic)
- Dynamic lambda$readLevelDataTagFixed$6(DataFixer,int,Dynamic)
+ Stream lambda$parseFeatureFlagsFromSummary$5(Dynamic)
- Stream lambda$parseFeatureFlagsFromSummary$7(Dynamic)
+ void lambda$parseFeatureFlagsFromSummary$6(ResourceLocation)
- void lambda$parseFeatureFlagsFromSummary$8(ResourceLocation)
```
