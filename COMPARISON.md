## Comparison with [1.18-pre1](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.18-pre1)
...
<table><tr><th></th><th align="left">1.18-pre1</th><th>1.18-pre2</th></tr><tr><td>World version</td><td><code>2847</code></td><td><code>2848</code></td></tr><tr><td>Protocol version</td><td><code>1073741873</code></td><td><code>1073741874</code></td></tr></table>
</details>
...
```
XXX.ai.behavior.AnimalPanic +3M | +1P -1P
```
```
...
```
```
XXX.level.biome.MobSpawnSettings +1M -2M | -1P
```
```
XXX.level.block.PowderSnowBlock +1M | +2P
```
```
...
- Optional lookForWater(BlockGetter,Entity)
- Vec3 getPanicPos(PathfinderMob,ServerLevel)
```
...
```diff
- LivingEntity$Fallsounds getFallSounds()
+ SoundEvent getFallDamageSound(int)
```
...
+ void <init>(float,Map,Map,boolean)
- void <init>(float,Map,Map)
```
...
```
XXX.server.commands.WorldBorderCommand +1P
```
```
XXX.server.players.GameProfileCache +1M
```
```
XXX.datafix.fixes.ChunkHeightAndBiomeFix +9M -4M
```
```
...
```diff
+ Dynamic lambda$makeBiomeContainer$20(Dynamic,Integer)
- Dynamic lambda$makeBiomeContainer$24(Dynamic,Integer)
- Dynamic lambda$shiftUpgradeData$20(Dynamic)
+ Dynamic lambda$updateHeightmaps$18(Dynamic)
- Dynamic lambda$updateHeightmaps$22(Dynamic)
- Dynamic shiftUpgradeData(Dynamic)
+ long lambda$getFixedHeightmap$19(long)
- long lambda$getFixedHeightmap$23(long)
+ Pair lambda$updateCarvingMasks$17(int,int,Dynamic,Pair)
- Pair lambda$updateCarvingMasks$21(int,int,Dynamic,Pair)
- void lambda$shiftUpgradeData$17(Map,Dynamic,Dynamic,Integer)
- void lambda$shiftUpgradeData$18(Map,Dynamic,Dynamic)
- void lambda$shiftUpgradeData$19(Map,Map)
```
...
```diff
- boolean isHorizontalCollisionMinor(Vec3)
```
