## Comparison with [1.19.1-pre2](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.19.1-pre2)
...
<table><tr><th></th><th align="left">1.19.1-pre2</th><th>1.19.1-pre3</th></tr><tr><td>World version</td><td><code>3110</code></td><td><code>3111</code></td></tr><tr><td>Protocol version</td><td><code>1073741919</code></td><td><code>1073741920</code></td></tr></table>
</details>
...
gui.socialInteractions.tooltip.report: Create report for %sReport player
gui.abuseReport.reason.terrorism_or_violent_extremism.description: Someone is talking about, promoting, or threatening to commwith acts of terrorism or violent extremism for political, religious, ideological, or other reasons.
...
gui.abuseReport.reason.imminent_harm.description: Someone is threatening to harm you or someone elsethemselves in real life or talking about harming themselves in real life.
...
```
XXX.network.chat.ChatSender +3M -2M | +1P -1P
```
```
XXX.network.chat.ChatTypeDecoration +2M -1M
```
```
XXX.network.chat.MessageSignature +1M -2M
```
```
XXX.protocol.game.ClientboundPlayerChatPacket +1M
```
```
XXX.minecraft.server.MinecraftServer +2M -1M
```
```
XXX.server.commands.MsgCommand +1M -1M
```
...
```diff
- ChatSender toSystem()
- ChatSender withTargetName(Component)
...
```diff
+ ChatTypeDecoration directMessage(String)
- ChatTypeDecoration incomingDirectMessage(String)
- ChatTypeDecoration outgoingDirectMessage(String)
```
...
- MessageSignature unsigned(UUID)
```
...
```diff
- ClientboundPlayerChatPacket system(Component,int,ChatSender,Instant)
```
...
```diff
- ChatType lambda$decorateChatMessage$24(ResourceKey,Registry)
+ ChatType lambda$logChatMessage$24(ResourceKey,Registry)
- Component decorateChatMessage(ChatSender,Component,ResourceKey)
```
...
```diff
- void lambda$sendMessage$1(Collection,ChatSender,CommandSourceStack,FilteredText)
+ void lambda$sendMessage$1(Collection,CommandSourceStack,FilteredText)
```
...
```
```
XXX.gui.components.ChatComponent +1M -4M | -2P
```
...
```diff
- int getLineHeight()
+ long getChatRateMillis()
+ void enqueueMessage(Component,GuiMessageTag)
+ void enqueueMessage(Component)
