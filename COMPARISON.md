## Comparison with [24w03b](https://github.com/PixiGeko/Minecraft-generated-data/tree/24w03b)
...
<table><tr><th></th><th align="left">24w03b</th><th>24w04a</th></tr><tr><td>DataPack version</td><td><code>28</code></td><td><code>29</code></td></tr><tr><td>World version</td><td><code>3805</code></td><td><code>3806</code></td></tr><tr><td>Protocol version</td><td><code>1073741996</code></td><td><code>1073741997</code></td></tr></table>
</details>
...
```
XXX.minecraft.core.RegistryCodecs +3M -8M
```
```
XXX.protocol.game.ClientboundDamageEventPacket +4M -4M | +2P -1P
```
```
XXX.protocol.game.ClientboundDisguisedChatPacket +2M -4M | +1P -1P
```
```
XXX.protocol.game.ClientboundPlayerChatPacket +4M -4M | +1P -1P
```
```
XXX.protocol.game.ClientboundRespawnPacket +2M -2M
```
```
XXX.protocol.game.CommonPlayerSpawnInfo +5M -4M | +2P -1P
```
```
XXX.minecraft.resources.RegistryDataLoader +17M -11M | +1P
```
```
XXX.minecraft.resources.RegistryDataLoader$Loader +9M | +3P -1P
```
```
XXX.server.network.ServerConfigurationPacketListenerImpl +2M -1M
```
```
XXX.server.players.PlayerList +2M -2M | +1P -1P
```
```
XXX.minecraft.tags.EntityTypeTags +1P
```
```
XXX.world.damagesource.DamageType +1P -1P
```
```
...
```
```
XXX.item.armortrim.TrimMaterials +3M -3M
```
```
XXX.item.armortrim.TrimPatterns +2M -2M
```
```
XXX.world.level.Level +3M -5M | -1P
```
```
XXX.world.level.NaturalSpawner -1M
```
```
XXX.level.levelgen.NoiseGeneratorSettings +6M -6M
```
```
XXX.level.levelgen.NoiseRouterData +3M -3M
```
```
XXX.levelgen.flat.FlatLevelGeneratorPresets$Bootstrap +1M -1M | +1P -1P
```
```
XXX.levelgen.presets.WorldPresets +1M -1M
```
```
XXX.structure.placement.StructurePlacement +2M
```
```
XXX.level.storage.DerivedLevelData +1M -1M
```
```
XXX.level.storage.WorldData +1P -1P
```
...
```diff
+ App lambda$withNameAndId$0(ResourceKey,MapCodec,RecordCodecBuilder$Instance)
+ Codec networkCodec(ResourceKey,Lifecycle,Codec)
+ List lambda$networkCodec$2(Registry)
- Map lambda$fullCodec$2(Registry)
+ Map lambda$fullCodec$5(Registry)
+ MapCodec withNameAndId(ResourceKey,MapCodec)
- Registry lambda$fullCodec$1(ResourceKey,Lifecycle,Map)
+ Registry lambda$fullCodec$4(ResourceKey,Lifecycle,Map)
+ Registry lambda$networkCodec$1(ResourceKey,Lifecycle,List)
- void lambda$fullCodec$0(WritableRegistry,Lifecycle,ResourceKey,Object)
...
- void <init>(int,Holder,int,int,Optional)
+ void <init>(int,int,int,int,Optional)
...
- void <init>(Component,ChatType$Bound)
+ void <init>(Component,ChatType$BoundNetwork)
+ void <init>(FriendlyByteBuf)
+ void write(FriendlyByteBuf)
```
...
+ void <init>(FriendlyByteBuf)
- void <init>(RegistryFriendlyByteBuf)
- void <init>(UUID,int,MessageSignature,SignedMessageBody$Packed,Component,FilterMask,ChatType$Bound)
+ void <init>(UUID,int,MessageSignature,SignedMessageBody$Packed,Component,FilterMask,ChatType$BoundNetwork)
+ void write(FriendlyByteBuf)
- void write(RegistryFriendlyByteBuf)
```
...
- void <init>(RegistryFriendlyByteBuf)
+ void <init>(ResourceKey,ResourceKey,long,GameType,GameType,boolean,boolean,Optional,int)
+ void write(FriendlyByteBuf)
- void write(RegistryFriendlyByteBuf)
```
...
```diff
+ Pair lambda$load$0(Map,RegistryDataLoader$RegistryData)
- RegistryAccess$Frozen load(Map,RegistryAccess,List)
- RegistryAccess$Frozen load(RegistryDataLoader$LoadingFunction,RegistryAccess,List)
- RegistryDataLoader$Loader lambda$load$2(Map,RegistryDataLoader$RegistryData)
+ ResourceLocation lambda$logErrors$5(Map$Entry)
+ ResourceLocation lambda$logErrors$6(Map$Entry)
- ResourceLocation lambda$logErrors$7(Map$Entry)
- ResourceLocation lambda$logErrors$8(Map$Entry)
+ void lambda$createContext$3(Map,RegistryAccess$RegistryEntry)
+ void lambda$createContext$4(Map,Pair)
- void lambda$createContext$5(Map,RegistryAccess$RegistryEntry)
- void lambda$createContext$6(Map,RegistryDataLoader$Loader)
- void lambda$load$0(ResourceManager,RegistryDataLoader$Loader,RegistryOps$RegistryInfoLookup)
- void lambda$load$1(Map,RegistryDataLoader$Loader,RegistryOps$RegistryInfoLookup)
+ void lambda$load$1(ResourceManager,RegistryOps$RegistryInfoLookup,Pair)
+ void lambda$load$2(Map,Pair)
- void lambda$load$3(RegistryDataLoader$LoadingFunction,RegistryOps$RegistryInfoLookup,RegistryDataLoader$Loader)
- void lambda$load$4(Map,RegistryDataLoader$Loader)
- void lambda$loadContentsFromManager$11(String)
- void lambda$loadContentsFromNetwork$12(String)
+ void lambda$loadRegistryContents$9(String)
- void lambda$logErrors$10(PrintWriter,Map$Entry)
+ void lambda$logErrors$7(PrintWriter,Map$Entry)
+ void lambda$logErrors$8(PrintWriter,Map$Entry)
- void lambda$logErrors$9(PrintWriter,Map$Entry)
- void loadContentsFromManager(ResourceManager,RegistryOps$RegistryInfoLookup,WritableRegistry,Decoder,Map)
- void loadContentsFromNetwork(Map,RegistryOps$RegistryInfoLookup,WritableRegistry,Decoder,Map)
+ void loadRegistryContents(RegistryOps$RegistryInfoLookup,ResourceManager,ResourceKey,WritableRegistry,Decoder,Map)
```
...
- String toString()
- void <init>(RegistryDataLoader$RegistryData,WritableRegistry,Map)
- void loadFromNetwork(Map,RegistryOps$RegistryInfoLookup)
- void loadFromResources(ResourceManager,RegistryOps$RegistryInfoLookup)
...
```diff
+ void lambda$addOptionalTasks$0(MinecraftServer$ServerResourcePackInfo)
- void lambda$addOptionalTasks$1(MinecraftServer$ServerResourcePackInfo)
- void lambda$startConfiguration$0(ResourceKey,List)
```
...
```diff
+ boolean isAllowCheatsForAllPlayers()
- boolean isAllowCommandsForAllPlayers()
+ void setAllowCheatsForAllPlayers(boolean)
- void setAllowCommandsForAllPlayers(boolean)
```
...
- void register(BootstrapContext,ResourceKey,Item,Style,float,Map)
- void register(BootstrapContext,ResourceKey,Item,Style,float)
```
...
+ void bootstrap(BootstapContext)
- void bootstrap(BootstrapContext)
+ void register(BootstapContext,Item,ResourceKey)
- void register(BootstrapContext,Item,ResourceKey)
```
...
```diff
- AbortableIterationConsumer$Continuation lambda$getEntities$1(Predicate,List,int,EntityTypeTest,Entity)
+ AbortableIterationConsumer$Continuation lambda$getEntities$2(Predicate,List,int,EntityTypeTest,Entity)
+ IllegalArgumentException lambda$new$0(Holder)
...
```diff
+ boolean isSpawnPositionOk(SpawnPlacements$Type,LevelReader,BlockPos,EntityType)
```
...
```diff
+ NoiseGeneratorSettings caves(BootstapContext)
- NoiseGeneratorSettings caves(BootstrapContext)
+ NoiseGeneratorSettings end(BootstapContext)
- NoiseGeneratorSettings end(BootstrapContext)
+ NoiseGeneratorSettings floatingIslands(BootstapContext)
- NoiseGeneratorSettings floatingIslands(BootstrapContext)
+ NoiseGeneratorSettings nether(BootstapContext)
- NoiseGeneratorSettings nether(BootstrapContext)
+ NoiseGeneratorSettings overworld(BootstapContext,boolean,boolean)
...
```diff
+ DensityFunction registerAndWrap(BootstapContext,ResourceKey,DensityFunction)
- DensityFunction registerAndWrap(BootstrapContext,ResourceKey,DensityFunction)
+ Holder bootstrap(BootstapContext)
- Holder bootstrap(BootstrapContext)
+ void registerTerrainNoises(BootstapContext,HolderGetter,DensityFunction,Holder,Holder,ResourceKey,ResourceKey,ResourceKey,ResourceKey,ResourceKey,boolean)
- void registerTerrainNoises(BootstrapContext,HolderGetter,DensityFunction,Holder,Holder,ResourceKey,ResourceKey,ResourceKey,ResourceKey,ResourceKey,boolean)
```
...
```diff
- boolean applyAdditionalChunkRestrictions(int,int,long)
- boolean applyInteractionsWithOtherStructures(ChunkGeneratorStructureState,int,int)
```
...
```
XXX.components.debugchart.AbstractDebugChart +5M -1M | +1P -1P
```
```
XXX.components.debugchart.FpsDebugChart +1M -1M
```
```
XXX.components.debugchart.TpsDebugChart +3M -1M | +3P
```
```
XXX.components.events.ContainerEventHandler -1M
```
```
XXX.client.multiplayer.ClientPacketListener +1M
```
```
XXX.client.multiplayer.PingDebugMonitor +1M -1M | +1P -1P
```
```
XXX.minecraft.core.RegistrySetBuilder$BuildState +1M -1M
```
```
XXX.minecraft.core.RegistrySynchronization +7M -15M | +1P -2P
```
```
XXX.data.tags.UpdateOneTwentyOneDamageTypes +1M -1M
```
```
XXX.data.worldgen.AncientCityStructurePools +1M -1M
```
```
XXX.data.worldgen.BastionHoglinStablePools +1M -1M
```
```
XXX.data.worldgen.BastionPieces +1M -1M
```
```
XXX.data.worldgen.BastionTreasureRoomPools +1M -1M
```
```
XXX.data.worldgen.DesertVillagePools +1M -1M
```
```
XXX.data.worldgen.NoiseData +3M -3M
```
```
XXX.data.worldgen.PlainVillagePools +1M -1M
```
```
XXX.data.worldgen.ProcessorLists +2M -2M
```
```
XXX.data.worldgen.SnowyVillagePools +1M -1M
```
```
XXX.data.worldgen.Structures +1M -1M
```
```
XXX.data.worldgen.TaigaVillagePools +1M -1M
```
```
XXX.data.worldgen.TrailRuinsStructurePools +1M -1M
```
```
XXX.data.worldgen.UpdateOneTwentyOnePools +2M -2M
```
```
XXX.data.worldgen.UpdateOneTwentyOneStructureSets +1M -1M
```
```
XXX.data.worldgen.VillagePools +1M -1M
```
```
XXX.worldgen.features.AquaticFeatures +1M -1M
```
```
XXX.worldgen.features.EndFeatures +1M -1M
```
```
XXX.worldgen.features.MiscOverworldFeatures +1M -1M
```
```
XXX.worldgen.features.OreFeatures +1M -1M
```
```
XXX.worldgen.features.TreeFeatures +1M -1M
```
```
XXX.worldgen.placement.AquaticPlacements +1M -1M
```
```
XXX.worldgen.placement.EndPlacements +1M -1M
```
```
XXX.worldgen.placement.NetherPlacements +1M -1M
```
```
XXX.worldgen.placement.PlacementUtils +3M -3M
```
```
XXX.worldgen.placement.VegetationPlacements +1M -1M
```
```
XXX.gametest.framework.GameTestHelper +1M
```
```
XXX.minecraft.network.Connection +2M -2M
```
```
XXX.network.chat.ChatType +1M -2M | +1P -1P
```
```
XXX.util.worldupdate.WorldUpgrader +1M -4M | +9P
```
```
XXX.world.entity.SpawnPlacements$Data +7M -1M | +1P -1P
```
```
XXX.level.biome.MultiNoiseBiomeSourceParameterLists +1M -1M
```
```
XXX.chunk.storage.ChunkStorage +2M -1M
```
```
XXX.chunk.storage.RegionFile +1M | +1P
```
```
XXX.chunk.storage.RegionFileVersion +5M -1M | +6P
```
```
XXX.chunk.storage.SectionStorage +2M -2M | +1P -3P
```
...
```diff
- long getValueForAggregation(int)
+ void <init>(Font,SampleLogger)
- void <init>(Font,SampleLogger)
- void drawAdditionalDimensions(GuiGraphics,int,int,int)
- void drawDimensions(GuiGraphics,int,int,int)
- void drawMainDimension(GuiGraphics,int,int,int)
```
...
```diff
+ void magicalSpecialHackyFocus(GuiEventListener)
```
...
```diff
- void updateTagsForRegistry(ResourceKey,TagNetworkSerialization$NetworkPayload)
```
...
```diff
+ void <init>(ClientPacketListener,SampleLogger)
- void <init>(ClientPacketListener,SampleLogger)
```
...
```diff
+ boolean lambda$ownedNetworkableRegistries$1(RegistryAccess$RegistryEntry)
- boolean lambda$ownedNetworkableRegistries$4(RegistryAccess$RegistryEntry)
+ Codec captureMap(UnboundedMapCodec)
+ Codec lambda$getNetworkCodec$2(RegistrySynchronization$NetworkedRegistryData)
...
- IllegalArgumentException lambda$packRegistry$1(Holder$Reference,String)
+ ImmutableMap lambda$static$0()
...
- void lambda$packRegistries$0(DynamicOps,RegistryAccess,BiConsumer,RegistryDataLoader$RegistryData)
- void lambda$packRegistry$2(RegistryDataLoader$RegistryData,DynamicOps,List,Holder$Reference)
- void lambda$packRegistry$3(RegistryDataLoader$RegistryData,DynamicOps,BiConsumer,Registry)
- void packRegistries(DynamicOps,RegistryAccess,BiConsumer)
- void packRegistry(DynamicOps,RegistryDataLoader$RegistryData,RegistryAccess,BiConsumer)
...
- void register(BootstrapContext,ResourceKey,int,double,double[])
...
- void register(BootstrapContext,ResourceKey,List)
```
...
- void register(BootstrapContext,String,StructureTemplatePool)
```
...
- void register(BootstrapContext,ResourceKey,Holder,List)
- void register(BootstrapContext,ResourceKey,Holder,PlacementModifier[])
```
...
+ void <init>(LevelStorageSource$LevelStorageAccess,DataFixer,Registry,boolean)
```
...
- String toString()
+ void <init>(Heightmap$Types,SpawnPlacements$Type,SpawnPlacements$SpawnPredicate)
- void <init>(Heightmap$Types,SpawnPlacementType,SpawnPlacements$SpawnPredicate)
```
...
- void handleLegacyStructureIndex(ChunkPos)
+ void write(ChunkPos,CompoundTag)
```
...
```diff
- InputStream lambda$static$6(InputStream)
- OutputStream lambda$static$7(OutputStream)
- RegionFileVersion getSelected()
+ void <init>(int,RegionFileVersion$StreamWrapper,RegionFileVersion$StreamWrapper)
- void <init>(int,String,RegionFileVersion$StreamWrapper,RegionFileVersion$StreamWrapper)
...
```diff
+ void <init>(Path,Function,Function,DataFixer,DataFixTypes,boolean,RegistryAccess,LevelHeightAccessor)
