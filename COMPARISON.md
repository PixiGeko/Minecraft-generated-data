## Comparison with [23w45a](https://github.com/PixiGeko/Minecraft-generated-data/tree/23w45a)
...
<table><tr><th></th><th align="left">23w45a</th><th>23w46a</th></tr><tr><td>DataPack version</td><td><code>24</code></td><td><code>25</code></td></tr><tr><td>World version</td><td><code>3690</code></td><td><code>3691</code></td></tr><tr><td>Protocol version</td><td><code>1073741986</code></td><td><code>1073741987</code></td></tr></table>
</details>
...
```diff
+ universal_tags/number_format_type.json
```
...
```
subtitles.entity.generic.wind_burst: Wind cCharge bursts
```
...
```
```
XXX.gametest.framework.TestCommand +8M -6M
```
```
XXX.network.chat.Component$Serializer -4M | -2P
```
```
XXX.protocol.game.ClientGamePacketListener +1P
```
```
XXX.world.scores.Score +11M -11M | +7P -6P
```
```
XXX.world.scores.ScoreboardSaveData +1M
```
...
- Typed readTypedOrThrow(Type,Dynamic,boolean)
...
```diff
+ BlockFamily$Builder featureLockedBehind(FeatureFlag[])
```
...
+ Field lambda$static$1()
+ int getPos(JsonReader)
...
- CompoundTag write()
+ int getScore()
+ int lambda$static$0(Score,Score)
- int value()
...
+ void <clinit>()
- void <init>()
+ void <init>(Scoreboard,Objective,String)
+ void add(int)
- void display(Component)
+ void increment()
- void lambda$read$1(Score,NumberFormat)
- void lambda$write$0(CompoundTag,Tag)
- void numberFormat(NumberFormat)
+ void reset()
+ void setScore(int)
- void value(int)
```
...
```diff
- void lambda$saveObjectives$1(CompoundTag,Tag)
```
...
```
XXX.gui.screens.RealmsPlayerScreen +1M -2M
```
```
XXX.gui.screens.RealmsSelectFileToUploadScreen +2M -1M
```
```
net.minecraft.Util +3M
```
```
XXX.minecraft.client.Camera +1M | +1P
```
```
XXX.client.gui.Gui +4M -1M | +1P
```
```
XXX.gui.components.AbstractSelectionList +2M -6M | -6P
```
```
XXX.gui.components.AbstractWidget +4M
```
```
XXX.gui.components.ContainerObjectSelectionList +2M -2M
```
```
XXX.gui.components.ObjectSelectionList +2M -2M
```
```
XXX.gui.components.OptionsList +1M -1M
```
```
XXX.gui.components.PlayerTabOverlay +1M -1M
```
```
XXX.commands.arguments.OperationArgument$Operation +1P -1P
```
```
XXX.protocol.game.ClientboundSetObjectivePacket +1M | +1P
```
```
XXX.minecraft.realms.RealmsObjectSelectionList +1M -1M
```
```
XXX.minecraft.server.ServerScoreboard +4M -3M
```
```
XXX.server.commands.ExecuteCommand +37M -36M
```
```
XXX.server.level.ServerLevel +1M -1M
```
```
XXX.server.level.ServerPlayer +3M -3M
```
```
XXX.level.block.CopperBulbBlock +1M -1M
```
```
XXX.providers.score.FixedScoreboardNameProvider +1M -1M
```
```
XXX.providers.score.ScoreboardNameProvider +1P -1P
```
```
XXX.world.scores.Objective +6M -1M | +2P
```
...
```diff
- Font access$200(RealmsPlayerScreen)
...
```diff
+ Font access$100(RealmsSelectFileToUploadScreen)
- Font access$400(RealmsSelectFileToUploadScreen)
- int access$100(int)
```
...
```diff
- boolean lambda$displayScoreboardSidebar$1(PlayerScoreEntry)
+ boolean lambda$displayScoreboardSidebar$1(Score)
- Gui$1DisplayEntry lambda$displayScoreboardSidebar$2(Scoreboard,NumberFormat,PlayerScoreEntry)
- Gui$1DisplayEntry[] lambda$displayScoreboardSidebar$3(int)
- void lambda$displayScoreboardSidebar$4(Gui$1DisplayEntry[],int,GuiGraphics,Component,int)
```
...
```diff
- void renderTablistScore(Objective,int,PlayerTabOverlay$ScoreDisplayEntry,int,int,UUID,GuiGraphics)
+ void renderTablistScore(Objective,int,String,int,int,UUID,GuiGraphics)
```
...
```diff
- void onPlayerRemoved(ScoreHolder)
+ void onPlayerRemoved(String)
- void onPlayerScoreRemoved(ScoreHolder,Objective)
+ void onPlayerScoreRemoved(String,Objective)
+ void onScoreChanged(Score)
- void onScoreChanged(ScoreHolder,Objective,Score)
- void onScoreLockChanged(ScoreHolder,Objective)
```
...
```diff
+ ArgumentBuilder lambda$addConditionals$57(CommandNode,boolean,DataCommands$DataProvider,ArgumentBuilder)
- ArgumentBuilder lambda$addConditionals$58(CommandNode,boolean,DataCommands$DataProvider,ArgumentBuilder)
+ boolean checkScore(CommandContext,BiPredicate)
- boolean checkScore(CommandContext,ExecuteCommand$IntBiPredicate)
+ boolean lambda$addConditionals$42(CommandContext)
- boolean lambda$addConditionals$42(int,int)
...
- boolean lambda$addConditionals$44(int,int)
...
- boolean lambda$addConditionals$46(int,int)
...
- boolean lambda$addConditionals$48(int,int)
...
- boolean lambda$addConditionals$50(int,int)
- boolean lambda$addConditionals$52(CommandContext)
+ boolean lambda$addConditionals$54(CommandContext)
- boolean lambda$addConditionals$55(CommandContext)
+ boolean lambda$expandOneToManyEntityRelation$73(Entity)
- boolean lambda$expandOneToManyEntityRelation$74(Entity)
+ boolean lambda$expandOneToOneEntityRelation$70(Entity)
- boolean lambda$expandOneToOneEntityRelation$71(Entity)
+ Collection lambda$addConditional$62(boolean,ExecuteCommand$CommandPredicate,CommandContext)
- Collection lambda$addConditional$63(boolean,ExecuteCommand$CommandPredicate,CommandContext)
+ Collection lambda$addConditionals$52(boolean,CommandContext)
- Collection lambda$addConditionals$53(boolean,CommandContext)
+ Collection lambda$addConditionals$55(boolean,DataCommands$DataProvider,CommandContext)
- Collection lambda$addConditionals$56(boolean,DataCommands$DataProvider,CommandContext)
+ Collection lambda$addIfBlocksConditional$65(boolean,boolean,CommandContext)
- Collection lambda$addIfBlocksConditional$66(boolean,boolean,CommandContext)
+ Collection lambda$expandOneToManyEntityRelation$74(Function,CommandContext)
- Collection lambda$expandOneToManyEntityRelation$75(Function,CommandContext)
+ Collection lambda$expandOneToOneEntityRelation$72(Function,CommandContext)
- Collection lambda$expandOneToOneEntityRelation$73(Function,CommandContext)
+ Component lambda$addConditional$63()
- Component lambda$addConditional$64()
+ Component lambda$checkIfRegions$68(OptionalInt)
- Component lambda$checkIfRegions$69(OptionalInt)
+ Component lambda$checkUnlessRegions$69()
- Component lambda$checkUnlessRegions$70()
+ Component lambda$createNumericConditionalHandler$58(int)
- Component lambda$createNumericConditionalHandler$59(int)
+ Component lambda$createNumericConditionalHandler$60()
- Component lambda$createNumericConditionalHandler$61()
+ int lambda$addConditional$64(boolean,ExecuteCommand$CommandPredicate,CommandContext)
...
+ int lambda$addConditionals$56(DataCommands$DataProvider,CommandContext)
- int lambda$addConditionals$57(DataCommands$DataProvider,CommandContext)
+ int lambda$addIfBlocksConditional$66(boolean,CommandContext)
...
+ List lambda$expandOneToOneEntityRelation$71(CommandSourceStack,Entity)
...
+ Stream lambda$createRelationOperations$82(Entity)
- Stream lambda$createRelationOperations$83(Entity)
+ void lambda$scheduleFunctionConditionsAndTest$83(IntPredicate,List,ExecutionCommandSource,boolean,int)
- void lambda$scheduleFunctionConditionsAndTest$84(IntPredicate,List,ExecutionCommandSource,boolean,int)
+ void lambda$scheduleFunctionConditionsAndTest$84(List,ExecutionCommandSource,ExecutionControl)
...
```diff
+ void tickIceAndSnow(boolean,BlockPos)
...
```diff
- void handleTeamKill(ScoreHolder,ScoreHolder,ObjectiveCriteria[])
+ void handleTeamKill(String,String,ObjectiveCriteria[])
+ void lambda$awardStat$10(int,Score)
- void lambda$awardStat$10(int,ScoreAccess)
+ void lambda$updateScoreForCriteria$3(int,Score)
- void lambda$updateScoreForCriteria$3(int,ScoreAccess)
```
...
```diff
- void checkAndFlip(BlockState,ServerLevel,BlockPos)
...
```diff
- ScoreHolder getScoreHolder(LootContext)
+ String getScoreboardName(LootContext)
```
...
- NumberFormat numberFormat()
- NumberFormat numberFormatOrDefault(NumberFormat)
- void <init>(Scoreboard,String,ObjectiveCriteria,Component,ObjectiveCriteria$RenderType,boolean,NumberFormat)
+ void <init>(Scoreboard,String,ObjectiveCriteria,Component,ObjectiveCriteria$RenderType)
- void setDisplayAutoUpdate(boolean)
