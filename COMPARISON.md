## Comparison with [22w03a](https://github.com/PixiGeko/Minecraft-generated-data/tree/22w03a)
...
<table><tr><th></th><th align="left">22w03a</th><th>22w05a</th></tr><tr><td>World version</td><td><code>2966</code></td><td><code>2967</code></td></tr><tr><td>Protocol version</td><td><code>1073741889</code></td><td><code>1073741890</code></td></tr></table>
</details>
...
```
XXX.minecraft.core.Vec3i +4M -4M
```
```
XXX.server.level.ChunkHolder +1M
```
```
XXX.server.level.ChunkMap +46M -43M | +2P
```
```
XXX.level.block.PointedDripstoneBlock -2M | -1P
```
```
XXX.levelgen.feature.DripstoneUtils +1M
```
...
- boolean closerToCenterThan(Position,double)
+ double distSqr(double,double,double,boolean)
+ double distSqr(Position,boolean)
+ double distSqr(Vec3i,boolean)
- double distToCenterSqr(double,double,double)
- double distToCenterSqr(Position)
- double distToLowCornerSqr(double,double,double)
```
...
- CompletableFuture lambda$schedule$15(ChunkHolder,ChunkAccess)
...
- Either lambda$prepareAccessibleChunk$41(Either)
+ Either lambda$prepareEntityTickingChunk$6(Either)
- Either lambda$prepareEntityTickingChunk$8(Either)
+ Either lambda$prepareTickingChunk$31(List)
+ Either lambda$prepareTickingChunk$32(Either)
- Either lambda$prepareTickingChunk$33(List)
...
- Integer lambda$dumpChunks$46(LevelChunk)
- Integer lambda$dumpChunks$47(LevelChunk)
+ LevelChunk lambda$prepareAccessibleChunk$38(List)
- LevelChunk lambda$prepareAccessibleChunk$40(List)
...
- Optional lambda$dumpChunks$44(ChunkAccess)
+ String lambda$printFuture$46(LevelChunk)
+ String lambda$printFuture$47(ChunkHolder$ChunkLoadingFailure)
- String lambda$printFuture$48(LevelChunk)
- String lambda$printFuture$49(ChunkHolder$ChunkLoadingFailure)
...
- void lambda$prepareAccessibleChunk$42(ChunkHolder,Runnable)
...
- void lambda$prepareTickingChunk$36(MutableObject,LevelChunk,ServerPlayer)
+ void lambda$prepareTickingChunk$37(ChunkHolder,Runnable)
- void lambda$prepareTickingChunk$37(ChunkPos,LevelChunk)
- void lambda$prepareTickingChunk$38(ChunkPos,Either)
- void lambda$prepareTickingChunk$39(ChunkHolder,Runnable)
...
- void lambda$scheduleUnload$13(ChunkHolder,CompletableFuture,long,ChunkAccess)
...
- void lambda$setViewDistance$43(ChunkPos,int,MutableObject,ServerPlayer)
```
...
+ void scheduleStalactiteFallTicks(BlockState,LevelAccessor,BlockPos)
```
...
```diff
- boolean isNeitherEmptyNorWater(BlockState)
```
...
```
XXX.gui.screens.CreateBuffetWorldScreen$BiomeList +3M -2M
```
```
XXX.renderer.block.BlockRenderDispatcher +1M -1M
```
```
XXX.renderer.block.LiquidBlockRenderer +8M -5M
```
```
XXX.ai.goal.PanicGoal +1M
```
```
XXX.entity.animal.Panda$PandaPanicGoal +1M -1M
```
```
XXX.entity.animal.PolarBear$PolarBearPanicGoal +1M -1M
```
...
- String lambda$new$1(CreateBuffetWorldScreen$BiomeList$Entry)
+ void lambda$new$1(Map$Entry)
- void lambda$new$2(CreateBuffetWorldScreen$BiomeList,AbstractSelectionList$Entry)
```
...
```diff
- boolean renderLiquid(BlockPos,BlockAndTintGetter,VertexConsumer,BlockState,FluidState)
+ boolean renderLiquid(BlockPos,BlockAndTintGetter,VertexConsumer,FluidState)
```
...
```diff
- boolean isFaceOccludedByNeighbor(BlockGetter,BlockPos,Direction,float,BlockState)
+ boolean isFaceOccludedByNeighbor(BlockGetter,BlockPos,Direction,float)
+ boolean isNeighborSameFluid(BlockGetter,BlockPos,Direction,FluidState)
- boolean isNeighborSameFluid(FluidState,FluidState)
- boolean shouldRenderFace(BlockAndTintGetter,BlockPos,FluidState,BlockState,Direction,FluidState)
+ boolean shouldRenderFace(BlockAndTintGetter,BlockPos,FluidState,BlockState,Direction)
- boolean tesselate(BlockAndTintGetter,BlockPos,VertexConsumer,BlockState,FluidState)
+ boolean tesselate(BlockAndTintGetter,BlockPos,VertexConsumer,FluidState)
- float calculateAverageHeight(BlockAndTintGetter,Fluid,float,float,float,BlockPos)
- float getHeight(BlockAndTintGetter,Fluid,BlockPos,BlockState,FluidState)
- float getHeight(BlockAndTintGetter,Fluid,BlockPos)
+ float getWaterHeight(BlockGetter,BlockPos,Fluid)
