## Comparison with [20w18a](https://github.com/PixiGeko/Minecraft-generated-data/tree/20w18a)
...
<table><tr><th></th><th align="left">20w18a</th><th>20w19a</th></tr><tr><td>World version</td><td><code>2532</code></td><td><code>2534</code></td></tr><tr><td>Protocol version</td><td><code>714</code></td><td><code>715</code></td></tr></table>
</details>
...
```
XXX.core.dispenser.OptionalDispenseItemBehavior +2M
```
```
XXX.minecraft.tags.BlockTags +1P
```
```
XXX.ai.util.RandomPos +1M
```
```
XXX.entity.animal.IronGolem +1M -2M
```
```
...
```
```
XXX.level.block.RedStoneWireBlock$1 +1P
```
```
...
```diff
- Vec3 getDismountLocationInDirection(Vec3,LivingEntity)
```
...
```
XXX.client.map.Map +4M -4M | +2P
```
```
XXX.client.map.Map$2 +2M -1M
```
```
XXX.util.thread.ProcessorHandle +2M
```
```
...
- Pair lambda$invalidateAndUpdate$3(int,int,BiomeSource,int,int,int[])
+ void lambda$invalidateAndUpdate$0(CompletableFuture)
- void lambda$invalidateAndUpdate$1(CompletableFuture)
- void lambda$new$0(MouseWheelEvent)
+ void lambda$null$1(int[],int[],int,int)
- void lambda$null$2(int[],int[],int,int)
```
...
```diff
- CompletableFuture askEither(Function)
- void lambda$askEither$0(CompletableFuture,Either)
```
...
```diff
+ boolean lambda$getDismountLocationForPassenger$8(BlockState)
```
...
```diff
+ EntityType$Builder instantDespawnDistance(int)
+ EntityType$Builder noDespawnDistance(int)
```
