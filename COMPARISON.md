## Comparison with [1.16-pre4](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.16-pre4)
...
<table><tr><th></th><th align="left">1.16-pre4</th><th>1.16-pre5</th></tr><tr><td>World version</td><td><code>2560</code></td><td><code>2561</code></td></tr><tr><td>Protocol version</td><td><code>727</code></td><td><code>729</code></td></tr></table>
</details>
...
```
options.graphics.fabulous.tooltip: "Fabulous"%s graphics uses screen shaders for drawing weather, clouds and particles behind translucent blocks and water.\nThis may severely impact performance for portable devices and 4K displays.
...
```
```
XXX.minecraft.client.Option +1M -1M | +3P
```
```
...
```
```
XXX.world.inventory.ItemCombinerMenu +1M
```
```
XXX.world.item.FireChargeItem -1M
```
```
XXX.world.item.FlintAndSteelItem -3M
```
```
XXX.item.crafting.RecipeManager +10M -8M
```
```
XXX.item.crafting.UpgradeRecipe +1M
```
```
XXX.world.level.CollisionSpliterator +2M | +1P
```
```
XXX.level.block.BaseFireBlock +2M
```
```
XXX.level.block.BedBlock -1M
```
```
XXX.block.state.StateDefinition +4M
```
...
```diff
- GpuWarnlistManager getGpuWarnlistManager()
```
...
```diff
+ boolean lambda$useOn$0(BlockBehaviour$BlockStateBase)
```
...
```diff
+ boolean canLightCampFire(BlockState)
+ boolean canUse(BlockState,LevelAccessor,BlockPos)
+ boolean lambda$canLightCampFire$2(BlockBehaviour$BlockStateBase)
```
...
+ Map lambda$null$9(RecipeType)
+ Recipe lambda$byKey$5(ResourceLocation,Map)
- Recipe lambda$byKey$6(ResourceLocation,Map)
- Recipe lambda$getAllRecipesFor$3(Recipe)
...
+ Stream lambda$getRecipesFor$3(RecipeType,Level,Container,Recipe)
- Stream lambda$getRecipesFor$4(RecipeType,Level,Container,Recipe)
+ String lambda$getRecipesFor$4(Recipe)
- String lambda$getRecipesFor$5(Recipe)
+ void lambda$replaceRecipes$10(Map,Recipe)
...
```diff
- boolean lambda$new$0(BlockState,BlockPos)
- void <init>(CollisionGetter,Entity,AABB,BiPredicate)
```
...
```diff
- MapCodec appendPropertyCodec(MapCodec,Supplier,String,Property)
