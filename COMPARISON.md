## Comparison with [1.19.1-rc2](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.19.1-rc2)
...
<table><tr><th></th><th align="left">1.19.1-rc2</th><th>1.19.1-rc3</th></tr><tr><td>World version</td><td><code>3115</code></td><td><code>3116</code></td></tr><tr><td>Protocol version</td><td><code>1073741924</code></td><td><code>1073741925</code></td></tr></table>
</details>
...
```
gui.socialInteractions.tooltip.report.disabled: Cannot create report:The reporting service is unavailable
gui.socialInteractions.tooltip.report.no_messages: Cannot create report: no recentNo reportable messages from player %s
chatPreview.warning.toast: This server uses Chat Preview and can see your messages as you type them, even before they're sentbefore they are sent to other players. You can turn this off in Chat Settings.
```
...
```
```
XXX.commands.arguments.MessageArgument +1M -2M
```
```
XXX.minecraft.network.Connection$PacketHolder +1M -1M | +1P -1P
```
```
XXX.network.chat.OutgoingPlayerChatMessage -1M | +1P -1P
```
```
XXX.network.chat.OutgoingPlayerChatMessage$Tracked +2M -1M
```
```
XXX.network.chat.SignedMessageChain$Decoder +2M | +1P
```
```
XXX.network.chat.SignedMessageValidator -1M
```
```
XXX.server.network.FilteredText +7M -15M | +3P -3P
```
```
XXX.server.network.ServerGamePacketListenerImpl +25M -30M
```
```
XXX.server.network.ServerLoginPacketListenerImpl +1M -1M
```
```
XXX.server.network.TextFilterClient +1M
```
...
+ void sendChatMessage(OutgoingPlayerChatMessage,ChatType$Bound)
...
+ Object lambda$logResolutionFailure$1(CommandSourceStack,Throwable)
+ PlayerChatMessage lambda$getChatMessage$0(MessageArgument$Message)
```
...
```diff
+ void <init>(Packet,GenericFutureListener)
- void <init>(Packet,PacketSendListener)
```
...
```diff
+ FilteredText createFromFiltered(FilteredText)
```
...
```diff
+ ClientboundPlayerChatPacket packetForPlayer(ServerPlayer,ChatType$Bound)
- Packet lambda$sendToPlayer$0()
- void sendToPlayer(ServerPlayer,boolean,ChatType$Bound)
```
...
+ FilteredText fullyFiltered(Object)
- FilteredText fullyFiltered(String)
+ FilteredText lambda$rebuildIfNeededAsync$0(Object,Object)
+ FilteredText map(Function,Function)
+ FilteredText map(Function)
+ FilteredText passThrough(Object)
- FilteredText passThrough(String)
+ FilteredText rebuildIfNeeded(Object,Function)
- FilterMask mask()
+ Object filter(CommandSourceStack,ServerPlayer)
+ Object filter(ServerPlayer,ServerPlayer)
+ Object filtered()
+ Object filteredOrElse(Object)
+ Object raw()
+ Object select(boolean)
- String filtered()
- String filteredOrEmpty()
- String raw()
+ void <init>(Object,Object)
- void <init>(String,FilterMask)
```
...
- CompletableFuture filterTextPacket(List)
- CompletableFuture filterTextPacket(Object,BiFunction)
+ CompletableFuture filterTextPacket(Object,Function,BiFunction)
+ CompletableFuture filterTextPacket(String,Function)
- CompletableFuture filterTextPacket(String)
...
+ void broadcastChatMessage(FilteredText)
- void broadcastChatMessage(PlayerChatMessage)
+ void filterTextPacket(List,Consumer)
+ void lambda$disconnect$0(Component,Future)
...
+ void send(Packet,GenericFutureListener)
...
+ void lambda$handleAcceptedLogin$0(ChannelFuture)
```
...
```diff
- FilterMask parseMask(String,JsonArray,TextFilterClient$IgnoreStrategy)
```
...
+ net.minecraft.client.gui.screens.reporting.ChatLogSegmenter$Results
...
- net.minecraft.client.multiplayer.chat.ChatListener$2
...
```
```
XXX.client.gui.Font +1M -1M | +1P
```
```
XXX.components.toasts.ToastComponent$ToastInstance +2M -2M | +2P
```
```
XXX.network.chat.ChatDecorator +2M -5M
```
...
```diff
+ boolean render(int,int,PoseStack)
- boolean render(int,PoseStack)
- void <init>(ToastComponent,Toast,int,int)
+ void <init>(ToastComponent,Toast)
```
