## Comparison with [23w43b](https://github.com/PixiGeko/Minecraft-generated-data/tree/23w43b)
...
<table><tr><th></th><th align="left">23w43b</th><th>23w44a</th></tr><tr><td>DataPack version</td><td><code>22</code></td><td><code>23</code></td></tr><tr><td>World version</td><td><code>3687</code></td><td><code>3688</code></td></tr><tr><td>Protocol version</td><td><code>1073741984</code></td><td><code>1073741985</code></td></tr></table>
</details>
...
```
```
XXX.commands.execution.CommandQueueEntry +2M -2M | +1P -1P
```
```
XXX.commands.execution.CustomModifierExecutor +1P -1P
```
```
XXX.commands.execution.EntryAction +1P -1P
```
```
XXX.commands.execution.ExecutionControl +1M | +1P -1P
```
```
XXX.execution.tasks.BuildContexts$Continuation +2M -2M | +2P -1P
```
```
XXX.execution.tasks.BuildContexts$Unbound +2M -2M
```
```
XXX.execution.tasks.ContinuationTask +3M -3M
```
```
XXX.execution.tasks.ExecuteCommand +3M -3M | +1P -1P
```
```
XXX.gametest.framework.TestCommand +2M
```
```
XXX.server.commands.DebugCommand$TraceCustomExecutor +3M -3M
```
```
XXX.server.commands.FunctionCommand +8M -2M
```
```
XXX.server.commands.ReturnCommand$ReturnFromCommandCustomModifier +2M -3M
```
...
```diff
+ void safeReplaceFile(File,File,File)
```
...
- Frame frame()
+ int depth()
- void <init>(Frame,EntryAction)
+ void <init>(int,EntryAction)
```
...
```diff
- ExecutionControl create(ExecutionContext,Frame)
```
...
```diff
+ void <init>(String,ContextChain,boolean,List)
- void <init>(String,ContextChain,ChainModifiers,ExecutionCommandSource,List)
- void execute(ExecutionContext,Frame)
+ void execute(ExecutionContext,int)
```
...
```diff
- void execute(ExecutionCommandSource,ExecutionContext,Frame)
+ void execute(ExecutionCommandSource,ExecutionContext,int)
- void execute(Object,ExecutionContext,Frame)
+ void execute(Object,ExecutionContext,int)
```
...
```diff
- void <init>(ContinuationTask$TaskProvider,List,Frame)
+ void <init>(ContinuationTask$TaskProvider,List,int)
- void execute(ExecutionContext,Frame)
+ void execute(ExecutionContext,int)
- void schedule(ExecutionContext,Frame,List,ContinuationTask$TaskProvider)
+ void schedule(ExecutionContext,int,List,ContinuationTask$TaskProvider)
```
...
```diff
+ void <init>(String,boolean,CommandContext)
- void <init>(String,ChainModifiers,CommandContext)
- void execute(ExecutionCommandSource,ExecutionContext,Frame)
+ void execute(ExecutionCommandSource,ExecutionContext,int)
- void execute(Object,ExecutionContext,Frame)
+ void execute(Object,ExecutionContext,int)
...
- int saveAndExportTestStructure(CommandSourceStack,StructureBlockEntity)
```
...
```diff
- void lambda$runGuarded$2(Collection,CommandSourceStack,int,String,ExecutionContext,Frame)
+ void lambda$runGuarded$2(Collection,CommandSourceStack,int,String,ExecutionContext,int)
+ void runGuarded(CommandSourceStack,ContextChain,boolean,ExecutionControl)
- void runGuarded(CommandSourceStack,ContextChain,ChainModifiers,ExecutionControl)
+ void runGuarded(ExecutionCommandSource,ContextChain,boolean,ExecutionControl)
- void runGuarded(ExecutionCommandSource,ContextChain,ChainModifiers,ExecutionControl)
```
...
```diff
- CommandResultCallback decorateOutputIfNeeded(ExecutionCommandSource,FunctionCommand$Callbacks,ResourceLocation,CommandResultCallback)
- void instantiateAndQueueFunctions(CompoundTag,ExecutionControl,CommandDispatcher,ExecutionCommandSource,CommandFunction,ResourceLocation,CommandResultCallback,boolean)
- void lambda$decorateOutputIfNeeded$6(FunctionCommand$Callbacks,ExecutionCommandSource,ResourceLocation,CommandResultCallback,boolean,int)
+ void lambda$queueFunctions$6(MutableInt,FunctionCommand$Callbacks,ExecutionCommandSource,ResourceLocation,int)
- void lambda$queueFunctionsNoReturn$7(FunctionCommand$1Accumulator,boolean,int)
- void lambda$queueFunctionsNoReturn$8(FunctionCommand$1Accumulator,CommandResultCallback,ExecutionContext,Frame)
- void queueFunctions(Collection,CompoundTag,ExecutionCommandSource,ExecutionCommandSource,ExecutionControl,FunctionCommand$Callbacks,ChainModifiers)
+ void queueFunctions(Collection,CompoundTag,ExecutionCommandSource,ExecutionCommandSource,ExecutionControl,FunctionCommand$Callbacks)
- void queueFunctionsAsReturn(Collection,CompoundTag,ExecutionCommandSource,ExecutionCommandSource,ExecutionControl,FunctionCommand$Callbacks)
- void queueFunctionsNoReturn(Collection,CompoundTag,ExecutionCommandSource,ExecutionCommandSource,ExecutionControl,FunctionCommand$Callbacks)
```
...
```diff
+ ExecutionCommandSource lambda$apply$1(ExecutionControl,ExecutionCommandSource)
- void apply(ExecutionCommandSource,List,ContextChain,ChainModifiers,ExecutionControl)
+ void apply(List,ContextChain,boolean,ExecutionControl)
- void apply(Object,List,ContextChain,ChainModifiers,ExecutionControl)
+ void lambda$apply$0(ExecutionControl,ExecutionCommandSource,boolean,int)
```
...
```
```
XXX.minecraft.client.HotbarManager +1M -1M | +1P -1P
```
```
XXX.client.model.BatModel +1M -1M
```
```
XXX.commands.execution.CustomCommandExecutor +1P -1P
```
```
XXX.commands.execution.CustomCommandExecutor$WithErrorHandling +3M -3M | +1P -1P
```
```
XXX.commands.execution.ExecutionContext +5M -2M
```
```
XXX.execution.tasks.BuildContexts$TopLevel +1M -1M
```
```
XXX.execution.tasks.CallFunction +4M -3M | +2P
```
```
XXX.execution.tasks.ContinuationTask$TaskProvider +1P -1P
```
```
XXX.gametest.framework.GameTestInfo +7M -6M | +3P
```
```
XXX.gametest.framework.GameTestServer +1P
```
```
XXX.gametest.framework.StructureUtils +3M -6M
```
```
XXX.minecraft.nbt.NbtIo +6M -5M
```
```
XXX.server.commands.DebugCommand$TraceCustomExecutor +3M -3M
```
...
```diff
+ void onError(CommandSyntaxException,ExecutionCommandSource,boolean,TraceCallbacks)
- void onError(CommandSyntaxException,ExecutionCommandSource,ChainModifiers,TraceCallbacks)
+ void run(ExecutionCommandSource,ContextChain,boolean,ExecutionControl)
- void run(ExecutionCommandSource,ContextChain,ChainModifiers,ExecutionControl)
+ void run(Object,ContextChain,boolean,ExecutionControl)
- void run(Object,ContextChain,ChainModifiers,ExecutionControl)
```
...
```diff
- Frame createTopFrame(ExecutionContext,CommandResultCallback)
- Frame$FrameControl frameControlForDepth(int)
- void lambda$frameControlForDepth$0(int)
- void queueInitialCommandExecution(ExecutionContext,String,ContextChain,ExecutionCommandSource,CommandResultCallback)
+ void queueInitialCommandExecution(ExecutionContext,String,ContextChain,ExecutionCommandSource)
- void queueInitialFunctionCall(ExecutionContext,InstantiatedFunction,ExecutionCommandSource,CommandResultCallback)
+ void queueInitialFunctionCall(InstantiatedFunction,Object)
```
...
```diff
- CommandQueueEntry lambda$execute$0(ExecutionCommandSource,Frame,UnboundEntryAction)
+ CommandQueueEntry lambda$execute$0(Object,int,UnboundEntryAction)
- void <init>(InstantiatedFunction,CommandResultCallback,boolean)
+ void <init>(InstantiatedFunction)
- void execute(ExecutionCommandSource,ExecutionContext,Frame)
- void execute(Object,ExecutionContext,Frame)
+ void execute(Object,ExecutionContext,int)
```
...
```diff
+ boolean lambda$succeed$4(Entity)
- boolean lambda$succeed$5(Entity)
- boolean lambda$tick$0(ChunkPos)
- void lambda$prepareTestStructure$7(GameTestListener)
+ void lambda$spawnStructure$6(GameTestListener)
+ void lambda$succeed$5(Entity)
- void lambda$succeed$6(Entity)
+ void lambda$tick$0(GameTestListener)
- void lambda$tick$2(GameTestListener)
+ void lambda$tickInternal$2(GameTestSequence)
- void lambda$tickInternal$4(GameTestSequence)
- void prepareTestStructure(BlockPos)
+ void spawnStructure(BlockPos)
```
...
```diff
+ CompoundTag tryLoadStructure(Path)
- IllegalStateException lambda$prepareTestStructure$0(String)
+ Stream intersectingChunks(BoundingBox,int)
+ StructureBlockEntity createStructureBlock(String,BlockPos,Rotation,ServerLevel,boolean)
...
+ StructureBlockEntity spawnStructure(String,BlockPos,Rotation,ServerLevel,boolean)
+ StructureTemplate getStructureTemplate(String,ServerLevel)
+ void lambda$forceLoadChunks$0(ServerLevel,ChunkPos)
```
...
- CompoundTag read(Path)
+ CompoundTag readCompressed(File,NbtAccounter)
- CompoundTag readCompressed(Path,NbtAccounter)
- DataOutputStream createCompressorStream(OutputStream)
+ void parseCompressed(File,StreamTagVisitor,NbtAccounter)
- void parseCompressed(Path,StreamTagVisitor,NbtAccounter)
+ void write(CompoundTag,File)
- void write(CompoundTag,Path)
+ void writeCompressed(CompoundTag,File)
- void writeCompressed(CompoundTag,Path)
```
