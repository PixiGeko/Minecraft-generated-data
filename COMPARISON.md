## Comparison with [19w42a](https://github.com/PixiGeko/Minecraft-generated-data/tree/19w42a)
...
<table><tr><th></th><th align="left">19w42a</th><th>19w44a</th></tr><tr><td>World version</td><td><code>2212</code></td><td><code>2213</code></td></tr><tr><td>Protocol version</td><td><code>559</code></td><td><code>560</code></td></tr></table>
</details>
...
```
XXX.mojang.math.Quaternion +2M | +1P
```
```
XXX.minecraft.core.Direction +1M
```
```
XXX.server.commands.ExecuteCommand +1M -1M
```
```
...
```
```
XXX.chunk.storage.ChunkStorage +3M | +1P
```
...
```diff
- boolean checkCustomPredicate(CommandSourceStack,LootItemCondition)
...
+ net.minecraft.client.renderer.entity.layers.VillagerTradeItemLayer
...
```
```
XXX.storage.loot.PredicateManager -1M
```
```
XXX.loot.predicates.LootItemCondition -2M | -1P
```
...
```diff
- void renderLevel(PoseStack,float,long,boolean,Camera,GameRenderer,LightTexture,Matrix4f)
+ void renderLevel(PoseStack,float,long,boolean,Camera,GameRenderer,LightTexture)
```
...
```diff
+ LootItemCondition get(ResourceLocation,LootItemCondition)
```
...
```diff
+ boolean lambda$static$0(LootContext)
+ void <clinit>()
```
