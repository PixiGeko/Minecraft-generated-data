...
```diff
+ Function TEXT_INTENSITY_POLYGON_OFFSET
+ Function TEXT_POLYGON_OFFSET
...
```diff
+ void <init>(LivingEntity,double,double,double,int)
- void <init>(LivingEntity,double,double,double)
```
...
```diff
+ ChunkGenerator val$chunkGenerator
- LevelHeightAccessor val$heightAccessor
...
```diff
+ BlockState FLOWERING_AZALEA_LEAVES
