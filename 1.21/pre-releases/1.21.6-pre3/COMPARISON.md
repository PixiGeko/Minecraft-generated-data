## Comparison with [1.21.6-pre2](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.21.6-pre2)
...
<br/>
<table><tr><th></th><th align="left">1.21.6-pre2</th><th>1.21.6-pre3</th></tr><tr><td>DataPack version</td><td><pre>79</pre></td><td><pre>80</pre></td></tr><tr><td>World version</td><td><pre>4431</pre></td><td><pre>4432</pre></td></tr><tr><td>Protocol version</td><td><pre>1073742077</pre></td><td><pre>1073742078</pre></td></tr></table>
</details>
<hr/>
...
```diff
+ playsound <sound: resource_location> ui <targets: entity> <pos: vec3> <volume: float> <pitch: float> <minVolume: float>
```
...
```diff
+ stopsound <targets: entity> ui <sound: resource_location>
```
...
```
```
XXX.advancements.critereon.PlayerInteractTrigger$TriggerInstance +2M
```
```
XXX.minecraft.sounds.SoundSource +1P
```
...
```diff
- Criterion equipmentSheared(ItemPredicate$Builder,Optional)
- Criterion equipmentSheared(Optional,ItemPredicate$Builder,Optional)
```
...
```
```
XXX.screens.dialog.DialogScreen$WarningScreen +4M -5M
```
```
XXX.client.renderer.LevelRenderer +13M -12M | +1P
```
...
+ Screen create(Minecraft,DialogConnectionAccess,DialogScreen)
- Screen create(Minecraft,Screen)
- Screen returnScreen()
+ void <init>(Minecraft,DialogConnectionAccess,MutableObject)
- void <init>(Minecraft,MutableObject)
+ void clearReturnScreen()
+ void updateReturnScreen(DialogScreen)
- void updateReturnScreen(Screen)
```
...
```diff
- Integer lambda$static$0(Entity)
...
- VertexConsumer lambda$renderBlockEntities$7(MultiBufferSource$BufferSource,VertexConsumer,RenderType)
+ void lambda$addCloudsPass$3(int,CloudStatus,float,Vec3,float)
- void lambda$addCloudsPass$4(int,CloudStatus,float,Vec3,float)
+ void lambda$addLateDebugPass$5(GpuBufferSlice,Vec3)
- void lambda$addLateDebugPass$6(GpuBufferSlice,Vec3)
+ void lambda$addMainPass$1(GpuBufferSlice,DeltaTracker,Camera,ProfilerFiller,Matrix4f,ResourceHandle,ResourceHandle,boolean,Frustum,ResourceHandle,ResourceHandle)
- void lambda$addMainPass$2(GpuBufferSlice,DeltaTracker,Camera,ProfilerFiller,Matrix4f,ResourceHandle,ResourceHandle,boolean,Frustum,ResourceHandle,ResourceHandle)
+ void lambda$addParticlesPass$2(GpuBufferSlice,ResourceHandle,ResourceHandle,Camera,float)
- void lambda$addParticlesPass$3(GpuBufferSlice,ResourceHandle,ResourceHandle,Camera,float)
+ void lambda$addSkyPass$8(GpuBufferSlice,DimensionSpecialEffects$SkyType,float,DimensionSpecialEffects)
- void lambda$addSkyPass$9(GpuBufferSlice,DimensionSpecialEffects$SkyType,float,DimensionSpecialEffects)
+ void lambda$addWeatherPass$4(GpuBufferSlice,float,Vec3,int,float)
- void lambda$addWeatherPass$5(GpuBufferSlice,float,Vec3,int,float)
+ void lambda$prepareChunkRenders$7(int,GpuBufferSlice[],RenderPass$UniformUploader)
- void lambda$prepareChunkRenders$8(int,GpuBufferSlice[],RenderPass$UniformUploader)
+ void lambda$renderLevel$0(Vector4f)
- void lambda$renderLevel$1(Vector4f)
```
