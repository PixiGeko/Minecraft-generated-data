## Comparison with [25w36b](https://github.com/PixiGeko/Minecraft-generated-data/tree/25w36b)
...
<br/>
<table><tr><th></th><th align="left">25w36b</th><th>25w37a</th></tr><tr><td>DataPack version</td><td><pre>{
  "major": 86,
  "minor": 0
}</pre></td><td><pre>{
  "major": 87,
  ...
  "major": 69,
  "minor": 0
}</pre></td></tr><tr><td>World version</td><td><pre>4546</pre></td><td><pre>4547</pre></td></tr><tr><td>Protocol version</td><td><pre>1073742091</pre></td><td><pre>1073742092</pre></td></tr></table>
</details>
<hr/>
...
```diff
+ universal_tags/debug_subscription.json
```
...
<br/>
<table>
<tr><th>Name</th><th>25w36b</th><th>25w37a</th></tr>
<tr><th align="left"><div style="width:290px">argument.block.tag.disallowed</div></th><td>Tags aren't allowed here; only actual blocks</td><td>Tags aren't allowed here; only actual blocks are</td></tr>
<tr><th align="left"><div style="width:290px">argument.item.tag.disallowed</div></th><td>Tags aren't allowed here; only actual items</td><td>Tags aren't allowed here; only actual items are</td></tr>
<tr><th align="left"><div style="width:290px">argument.range.ints</div></th><td>Only whole numbers allowed; not decimals</td><td>Only whole numbers are allowed; not decimals</td></tr>
<tr><th align="left"><div style="width:290px">commands.data.modify.expected_list</div></th><td>Expected list; got: %s</td><td>Expected a list: got %s</td></tr>
<tr><th align="left"><div style="width:290px">commands.data.modify.expected_object</div></th><td>Expected object; got: %s</td><td>Expected an object: got %s</td></tr>
<tr><th align="left"><div style="width:290px">commands.data.modify.expected_value</div></th><td>Expected value; got: %s</td><td>Expected a value: got %s</td></tr>
...
- XXX.jsonrpc.internalapi.MinecraftApi$MinecraftApiBuilder
...
```
net.minecraft.SharedConstants +4M | +8P -7P
```
```
net.minecraft.SystemReport +24M -20M
```
```
net.minecraft.Util +9M -11M
```
```
XXX.minecraft.core.SectionPos +1M | +1P
```
```
...
```
```
XXX.gametest.framework.GameTestRunner -1M
```
```
XXX.gametest.framework.ReportGameListener -1M
```
```
...
```
```
XXX.protocol.game.ServerGamePacketListener +1P -1P
```
```
XXX.server.jsonrpc.Connection +7M -5M
```
```
XXX.server.jsonrpc.JsonRPCErrors +3M -2M
```
```
XXX.server.jsonrpc.ManagementServer +7M -3M | +3P
```
```
XXX.server.level.ServerPlayer +2M | +1P
```
```
XXX.server.level.ServerPlayerGameMode +1M -1M | +1P -1P
```
```
...
```
```
XXX.util.debugchart.RemoteSampleLogger +2M -2M | +1P -1P
```
```
XXX.ai.navigation.PathNavigation +1M
```
```
XXX.village.poi.PoiManager +3M -3M
```
```
XXX.entity.animal.Bee +2M -1M
```
```
...
```
```
XXX.animal.coppergolem.CopperGolem -1M
```
```
XXX.animal.frog.Tadpole -1M
```
```
XXX.animal.sniffer.Sniffer -1M
```
```
XXX.entity.decoration.Mannequin +1M | +1P
```
```
XXX.entity.monster.Zoglin -1M
```
```
XXX.monster.breeze.Breeze +2M -1M
```
```
...
```
```
XXX.world.inventory.SlotRanges +4M -3M
```
```
XXX.world.level.GameRules +8M -7M | +2P -1P
```
```
XXX.world.level.Level +1P -1P
```
```
XXX.level.block.ShelfBlock +1M -1M
```
```
XXX.block.entity.BlockEntity +1M
```
```
XXX.block.entity.TestInstanceBlockEntity +3M | +1P
```
...
- boolean debugFlag(String)
- int debugIntValue(String)
- String prefixDebugFlagName(String)
```
...
- boolean lambda$new$5(String)
+ String lambda$new$3()
- String lambda$new$4()
- String lambda$new$6()
+ String lambda$putGraphics$15(GraphicsCard)
- String lambda$putGraphics$18(GraphicsCard)
- String lambda$putPhysicalMemory$10(PhysicalMemory)
- String lambda$putPhysicalMemory$11(PhysicalMemory)
+ String lambda$putPhysicalMemory$7(PhysicalMemory)
+ String lambda$putPhysicalMemory$8(PhysicalMemory)
+ String lambda$putProcessor$16(CentralProcessor$ProcessorIdentifier)
+ String lambda$putProcessor$17(CentralProcessor)
+ String lambda$putProcessor$18(CentralProcessor)
+ String lambda$putProcessor$19(CentralProcessor)
- String lambda$putProcessor$19(CentralProcessor$ProcessorIdentifier)
- String lambda$putProcessor$20(CentralProcessor)
- String lambda$putProcessor$21(CentralProcessor)
- String lambda$putProcessor$22(CentralProcessor)
+ String lambda$putSpaceForProperty$21(String)
- String lambda$putSpaceForProperty$24(String)
+ String lambda$putStorage$20()
- String lambda$putStorage$23()
+ String lambda$putVirtualMemory$10(VirtualMemory)
+ String lambda$putVirtualMemory$11(VirtualMemory)
- String lambda$putVirtualMemory$13(VirtualMemory)
- String lambda$putVirtualMemory$14(VirtualMemory)
- String lambda$putVirtualMemory$15(VirtualMemory)
+ String lambda$putVirtualMemory$9(VirtualMemory)
+ String lambda$toLineSeparatedString$23(Map$Entry)
- String lambda$toLineSeparatedString$26(Map$Entry)
- String printJvmFlags(Predicate)
+ void lambda$appendToCrashReportString$22(StringBuilder,String,String)
- void lambda$appendToCrashReportString$25(StringBuilder,String,String)
+ void lambda$putHardware$4(HardwareAbstractionLayer)
+ void lambda$putHardware$5(HardwareAbstractionLayer)
+ void lambda$putHardware$6(HardwareAbstractionLayer)
- void lambda$putHardware$7(HardwareAbstractionLayer)
- void lambda$putHardware$8(HardwareAbstractionLayer)
- void lambda$putHardware$9(HardwareAbstractionLayer)
+ void lambda$putMemory$13(GlobalMemory)
+ void lambda$putMemory$14(GlobalMemory)
- void lambda$putMemory$16(GlobalMemory)
- void lambda$putMemory$17(GlobalMemory)
```
...
```diff
+ boolean lambda$getVmArguments$19(String)
- List lambda$fallibleSequence$24(List,Void)
+ List lambda$fallibleSequence$25(List,Void)
- List lambda$sequence$20(List,Void)
+ List lambda$sequence$21(List,Void)
- Object lambda$mapValues$19(Function,Map$Entry)
+ Object lambda$mapValues$20(Function,Map$Entry)
+ Stream getVmArguments()
- String lambda$fixedSize$26(int)
+ String lambda$fixedSize$29(int)
- String lambda$sanitizeName$29(CharPredicate,int)
+ String lambda$sanitizeName$30(CharPredicate,int)
- void lambda$fallibleSequence$22(Consumer,List,int,Object,Throwable)
+ void lambda$fallibleSequence$23(Consumer,List,int,Object,Throwable)
- void lambda$fallibleSequence$23(List,CompletableFuture[],Consumer,CompletableFuture)
+ void lambda$fallibleSequence$24(List,CompletableFuture[],Consumer,CompletableFuture)
- void lambda$prefix$25(Consumer,String,String)
+ void lambda$prefix$26(Consumer,String,String)
- void lambda$sequenceFailFastAndCancel$21(CompletableFuture,List,Throwable)
+ void lambda$sequenceFailFastAndCancel$22(CompletableFuture,List,Throwable)
```
...
```diff
+ FriendlyByteBuf writeSectionPos(SectionPos)
...
```diff
- JsonArray handleBatchRequest(List)
+ JsonElement dispatchIncomingRequest(Integer,String,JsonElement)
- JsonElement dispatchIncomingRequest(JsonElement,String,JsonElement)
+ JsonElement lambda$dispatchIncomingRequest$1(Optional,JsonElement)
- JsonElement lambda$dispatchIncomingRequest$2(Optional,JsonElement)
+ JsonObject handleError(Integer,JsonObject)
- JsonObject handleError(JsonElement,JsonObject)
+ JsonObject handleIncomingRequest(Integer,String,JsonElement)
- JsonObject handleIncomingRequest(JsonElement,String,JsonElement)
- JsonObject lambda$handleBatchRequest$1(JsonElement)
+ void handleRequestResponse(int,JsonElement)
- void handleRequestResponse(JsonElement,JsonElement)
```
...
```diff
+ JsonObject create(Integer,String)
+ JsonObject create(Integer)
- JsonObject create(JsonElement,String)
- JsonObject create(JsonElement)
- JsonObject createWithMessage(String)
```
...
```diff
- int getPort()
+ MinecraftApi getApiService(DedicatedServer,JsonRpcLogger)
- void <init>(HostAndPort,AuthenticationHandler,NioEventLoopGroup)
- void <init>(HostAndPort,AuthenticationHandler)
+ void <init>(HostAndPort)
+ void start(DedicatedServer)
- void start(MinecraftApi,SslContext)
- void startWithoutTls(MinecraftApi)
- void startWithTls(MinecraftApi,SslContext)
- void stop(boolean)
```
...
```diff
- Set debugSubscriptions()
- void requestDebugSubscriptions(Set)
```
...
```diff
- boolean isInRangeOfGround()
+ void setGameModeForPlayer(GameType,GameType,TriState)
```
...
```diff
+ void handleDebugSampleSubscription(ServerboundDebugSampleSubscriptionPacket)
- void handleDebugSubscriptionRequest(ServerboundDebugSubscriptionRequestPacket)
```
...
```diff
+ void <init>(int,DebugSampleSubscriptionTracker,RemoteDebugSampleType,long[])
+ void <init>(int,DebugSampleSubscriptionTracker,RemoteDebugSampleType)
- void <init>(int,ServerDebugSubscribers,RemoteDebugSampleType,long[])
- void <init>(int,ServerDebugSubscribers,RemoteDebugSampleType)
```
...
```diff
- boolean lambda$new$0(ServerDebugSubscribers)
```
...
```diff
- DebugPoiInfo getDebugPoiInfo(BlockPos)
+ int getFreeTickets(BlockPos)
+ Integer lambda$getFreeTickets$21(BlockPos,PoiSection)
- Optional lambda$getDebugPoiInfo$21(BlockPos,PoiSection)
- PoiRecord add(BlockPos,Holder)
+ void add(BlockPos,Holder)
```
...
```diff
- DebugBeeInfo lambda$registerDebugValues$1()
- void registerDebugValues(ServerLevel,DebugValueSource$Registration)
...
```diff
- Mannequin create(EntityType,Level)
```
...
```diff
- DebugBreezeInfo lambda$registerDebugValues$3()
- void registerDebugValues(ServerLevel,DebugValueSource$Registration)
+ void sendDebugPackets()
```
...
```diff
+ boolean lambda$singleSlotNames$3(SlotRange)
- boolean lambda$singleSlotNames$4(SlotRange)
+ SlotRange[] lambda$static$1()
- SlotRange[] lambda$static$1(int)
- SlotRange[] lambda$static$2()
- SlotRange[] lambda$static$3(int)
+ String lambda$static$2(String)
```
...
```diff
+ boolean lambda$availableRules$14(FeatureFlagSet,Map$Entry)
- boolean lambda$availableRules$15(FeatureFlagSet,Map$Entry)
+ GameRules$Value lambda$copy$17(Map$Entry)
- GameRules$Value lambda$copy$18(Map$Entry)
+ GameRules$Value lambda$new$13(Map$Entry)
- GameRules$Value lambda$new$14(Map$Entry)
+ void lambda$assignFrom$19(GameRules,MinecraftServer,GameRules$Key)
- void lambda$assignFrom$20(GameRules,MinecraftServer,GameRules$Key)
+ void lambda$createTag$15(CompoundTag,GameRules$Key,GameRules$Value)
- void lambda$createTag$16(CompoundTag,GameRules$Key,GameRules$Value)
+ void lambda$loadFromTag$16(DynamicLike,GameRules$Key,GameRules$Value)
- void lambda$loadFromTag$17(DynamicLike,GameRules$Key,GameRules$Value)
- void lambda$static$13(MinecraftServer,GameRules$BooleanValue)
+ void lambda$visitGameRuleTypes$18(GameRules$GameRuleTypeVisitor,GameRules$Key,GameRules$Type)
- void lambda$visitGameRuleTypes$19(GameRules$GameRuleTypeVisitor,GameRules$Key,GameRules$Type)
```
...
```diff
- boolean swapHotbar(Level,BlockPos,Inventory)
+ void swapHotbar(Level,BlockPos,Inventory)
```
...
```diff
- List getErrorMarkers()
- void clearErrorMarkers()
- void markError(BlockPos,Component)
```
...
- XXX.client.renderer.QuadParticleRenderState$PreparedLayer
...
- XXX.entity.decoration.MannequinProfile
...
```
XXX.minecraft.client.Minecraft +27M -28M
```
```
XXX.client.entity.ClientMannequin +7M -4M | +1P
```
```
XXX.client.renderer.GameRenderer +1M
```
```
XXX.client.renderer.SkyRenderer +23M -10M | +16P
```
```
XXX.client.renderer.SubmitNodeCollector$ParticleGroupRenderer +2P -1P
```
```
XXX.renderer.blockentity.BannerRenderer +2M -3M
```
```
XXX.renderer.blockentity.BlockEntityRenderers +1M -2M
```
```
XXX.blockentity.state.TestInstanceRenderState +1P
```
```
XXX.renderer.debug.BeeDebugRenderer +23M -36M | -5P
```
```
XXX.renderer.debug.BreezeDebugRenderer +5M -9M | -1P
```
```
XXX.renderer.debug.ChunkCullingDebugRenderer +1M -1M
```
```
XXX.renderer.debug.DebugRenderer +2M | +2P -1P
```
```
XXX.renderer.debug.PathfindingRenderer +4M -2M | +2P -4P
```
```
XXX.renderer.feature.ParticleFeatureRenderer +6M -1M | +2P
```
```
XXX.jsonrpc.internalapi.MinecraftAllowListService -6M | +5P -2P
```
```
XXX.jsonrpc.internalapi.MinecraftBanListService -8M | +8P -2P
```
```
XXX.jsonrpc.internalapi.MinecraftOperatorListService -6M | +5P -2P
```
```
XXX.jsonrpc.internalapi.MinecraftServerSettingsService -41M | +40P -2P
```
```
XXX.jsonrpc.methods.AllowlistService +1M -1M
```
```
XXX.jsonrpc.methods.OperatorService +1M -1M
```
```
XXX.level.chunk.LevelChunk +6M -3M
```
```
XXX.level.gameevent.BlockPositionSource +5M -3M
```
```
XXX.levelgen.structure.BoundingBox +3M | +1P
```
```
XXX.level.pathfinder.PathFinder +5M -3M | +1P -1P
```
```
XXX.level.redstone.CollectingNeighborUpdater +1M | +1P
```
```
XXX.level.redstone.CollectingNeighborUpdater$MultiNeighborUpdate +1M
```
```
XXX.level.redstone.CollectingNeighborUpdater$ShapeUpdate +1M
```
```
...
```diff
- boolean lambda$doWorldLoad$38(long)
...
- IntegratedServer lambda$doWorldLoad$36(LevelStorageSource$LevelStorageAccess,PackRepository,WorldStem,LevelLoadListener,Thread)
+ IntegratedServer lambda$doWorldLoad$37(LevelStorageSource$LevelStorageAccess,PackRepository,WorldStem,LevelLoadListener,Thread)
- Optional lambda$getQuickActionsDialog$35(Registry,HolderSet$Named)
+ Optional lambda$getQuickActionsDialog$36(Registry,HolderSet$Named)
...
- void lambda$handleKeybinds$34(Holder)
+ void lambda$handleKeybinds$35(Holder)
...
```diff
- Mannequin lambda$registerOverrides$0(PlayerSkinRenderCache,EntityType,Level)
+ Optional lambda$onSyncedDataUpdated$1(Optional)
- Optional lambda$updateSkin$1(Optional)
+ void lambda$onSyncedDataUpdated$2(ResolvableProfile)
+ void lambda$onSyncedDataUpdated$3(MannequinProfile)
+ void lambda$tick$0(PlayerSkin)
- void lambda$updateSkin$2(ResolvableProfile)
- void lambda$updateSkin$3(MannequinProfile)
- void registerOverrides(PlayerSkinRenderCache)
- void setSkin(PlayerSkin)
- void updateSkin()
```
...
```diff
- Matrix4f getProjectionMatrixForCulling(float)
```
...
```diff
- AbstractTexture getTexture(ResourceLocation)
- GpuBuffer buildEndFlashQuad()
- GpuBuffer buildMoonPhases()
- GpuBuffer buildSunQuad()
- GpuBuffer buildSunriseFan()
- String lambda$buildEndFlashQuad$7()
- String lambda$buildMoonPhases$4()
+ String lambda$buildStars$2()
- String lambda$buildStars$5()
- String lambda$buildSunQuad$3()
- String lambda$buildSunriseFan$2()
+ String lambda$renderDarkDisc$4()
- String lambda$renderDarkDisc$9()
- String lambda$renderEndFlash$15()
- String lambda$renderEndSky$14()
+ String lambda$renderEndSky$7()
- String lambda$renderMoon$11()
+ String lambda$renderSkyDisc$3()
- String lambda$renderSkyDisc$8()
- String lambda$renderStars$12()
+ String lambda$renderStars$5()
- String lambda$renderSun$10()
- String lambda$renderSunriseAndSunset$13()
+ void renderEndFlash(MultiBufferSource,PoseStack,float,float,float)
- void renderEndFlash(PoseStack,float,float,float)
+ void renderMoon(int,float,MultiBufferSource,PoseStack)
- void renderMoon(int,float,PoseStack)
+ void renderSun(float,MultiBufferSource,PoseStack)
- void renderSun(float,PoseStack)
- void renderSunMoonAndStars(PoseStack,float,int,float,float)
+ void renderSunMoonAndStars(PoseStack,MultiBufferSource$BufferSource,float,int,float,float)
- void renderSunriseAndSunset(PoseStack,float,int)
+ void renderSunriseAndSunset(PoseStack,MultiBufferSource$BufferSource,float,int)
```
...
```diff
- void submitPatternLayer(MaterialSet,PoseStack,SubmitNodeCollector,int,int,Model,Object,Material,DyeColor,ModelFeatureRenderer$CrumblingOverlay)
...
- void lambda$createEntityRenderers$5(ImmutableMap$Builder,BlockEntityRendererProvider$Context,BlockEntityType,BlockEntityRendererProvider)
...
- Collection getHiveMembers(BlockPos,DebugValueAccess)
+ Collection getHiveMembers(BlockPos)
+ List lambda$getGhostHives$13(BlockPos)
- List lambda$getGhostHives$9(BlockPos)
+ Map createHiveBlacklistMap()
- Map createHiveBlacklistMap(DebugValueAccess)
+ Map getGhostHives()
- Map getGhostHives(DebugValueAccess)
+ Set getHiveMemberNames(HiveDebugPayload$HiveInfo)
- Set lambda$createHiveBlacklistMap$3(BlockPos)
+ Set lambda$createHiveBlacklistMap$6(BlockPos)
- Set lambda$renderFlowerInfos$5(BlockPos)
+ Set lambda$renderFlowerInfos$9(BlockPos)
+ String getPosDescription(BeeDebugPayload$BeeInfo,BlockPos)
- String getPosDescription(Entity,BlockPos)
...
+ void clearRemovedHives()
- void doRender(PoseStack,MultiBufferSource,DebugValueAccess)
+ void doRender(PoseStack,MultiBufferSource)
- void lambda$createHiveBlacklistMap$4(Map,Entity,DebugBeeInfo)
...
- void lambda$doRender$0(DebugValueAccess,PoseStack,MultiBufferSource,Entity,DebugBeeInfo)
- void lambda$doRender$1(BlockPos,PoseStack,MultiBufferSource,Map,DebugValueAccess,BlockPos,DebugHiveInfo)
- void lambda$doRender$2(BlockPos,PoseStack,MultiBufferSource,BlockPos,List)
...
+ void lambda$doRender$5(BlockPos,PoseStack,MultiBufferSource,BlockPos,List)
- void lambda$getGhostHives$10(DebugValueAccess,Map,Entity,DebugBeeInfo)
- void lambda$getHiveMembers$8(BlockPos,Set,Entity,DebugBeeInfo)
...
- void lambda$renderFlowerInfos$6(Map,Entity,DebugBeeInfo)
- void lambda$renderFlowerInfos$7(PoseStack,MultiBufferSource,BlockPos,Set)
- void lambda$updateLastLookedAtUuid$11(Entity)
+ void lambda$updateLastLookedAtUuid$14(Entity)
+ void removeBeeInfo(int)
- void render(PoseStack,MultiBufferSource,double,double,double,DebugValueAccess)
+ void render(PoseStack,MultiBufferSource,double,double,double)
+ void renderBeeInfo(PoseStack,MultiBufferSource,BeeDebugPayload$BeeInfo)
- void renderBeeInfo(PoseStack,MultiBufferSource,Entity,DebugBeeInfo,DebugGoalInfo)
- void renderFlowerInfos(PoseStack,MultiBufferSource,DebugValueAccess)
+ void renderFlowerInfos(PoseStack,MultiBufferSource)
- void renderHiveInfo(PoseStack,MultiBufferSource,BlockPos,DebugHiveInfo,Collection,DebugValueAccess)
...
- void renderTextOverHive(PoseStack,MultiBufferSource,String,BlockPos,DebugHiveInfo,int,int)
...
```diff
+ boolean lambda$render$0(Breeze)
+ Entity lambda$render$1(LocalPlayer,Integer)
- Vec3 lambda$render$0(Entity)
+ Vec3 lambda$render$2(Entity)
+ void add(BreezeDebugPayload$BreezeInfo)
+ void clear()
- void lambda$render$1(PoseStack,MultiBufferSource,double,double,double,Entity,Vec3)
- void lambda$render$2(PoseStack,MultiBufferSource,double,double,double,Entity,BlockPos)
- void lambda$render$3(ClientLevel,PoseStack,MultiBufferSource,double,double,double,Entity,DebugBreezeInfo)
+ void lambda$render$3(PoseStack,MultiBufferSource,double,double,double,Breeze,Vec3)
+ void lambda$render$4(PoseStack,MultiBufferSource,double,double,double,Breeze,BlockPos)
+ void lambda$render$5(LocalPlayer,PoseStack,MultiBufferSource,double,double,double,Breeze)
- void render(PoseStack,MultiBufferSource,double,double,double,DebugValueAccess)
+ void render(PoseStack,MultiBufferSource,double,double,double)
```
...
```diff
- void renderTextOverBlock(PoseStack,MultiBufferSource,String,BlockPos,int,int,float)
- void renderTextOverMob(PoseStack,MultiBufferSource,Entity,int,String,int,float)
```
...
- void close()
- void endFrame()
- void prepareRenderPass(RenderPass)
+ void render(SubmitNodeCollection,MultiBufferSource$BufferSource)
- void render(SubmitNodeCollection)
```
...
```diff
+ boolean add(UserWhiteListEntry,ClientInfo)
+ Collection getEntries()
+ void <init>(DedicatedServer,JsonRpcLogger)
+ void clear(ClientInfo)
+ void kickUnlistedPlayers(ClientInfo)
+ void remove(NameAndId,ClientInfo)
```
...
+ void <init>(MinecraftServer,JsonRpcLogger)
+ void addIpBan(IpBanListEntry,ClientInfo)
+ void addUserBan(UserBanListEntry,ClientInfo)
+ void clearIpBans(ClientInfo)
+ void removeIpBan(String,ClientInfo)
+ void removeUserBan(NameAndId,ClientInfo)
```
...
+ void <init>(MinecraftServer,JsonRpcLogger)
+ void clear(ClientInfo)
+ void deop(NameAndId,ClientInfo)
+ void op(NameAndId,ClientInfo)
+ void op(NameAndId,Optional,Optional,ClientInfo)
```
...
+ boolean setAcceptsTransfers(boolean,ClientInfo)
+ boolean setAllowFlight(boolean,ClientInfo)
+ boolean setAutoSave(boolean,ClientInfo)
+ boolean setEnforceWhitelist(boolean,ClientInfo)
+ boolean setForceGameMode(boolean,ClientInfo)
+ boolean setHidesOnlinePlayers(boolean,ClientInfo)
+ boolean setRepliesToStatus(boolean,ClientInfo)
+ boolean setUsingWhitelist(boolean,ClientInfo)
+ Difficulty getDifficulty()
+ Difficulty setDifficulty(Difficulty,ClientInfo)
+ GameType getGameMode()
+ GameType setGameMode(GameType,ClientInfo)
...
+ int getViewDistance()
+ int setEntityBroadcastRangePercentage(int,ClientInfo)
+ int setMaxPlayers(int,ClientInfo)
+ int setOperatorUserPermissionLevel(int,ClientInfo)
+ int setPauseWhenEmptySeconds(int,ClientInfo)
+ int setPlayerIdleTimeout(int,ClientInfo)
+ int setSimulationDistance(int,ClientInfo)
+ int setSpawnProtectionRadius(int,ClientInfo)
+ int setStatusHeartbeatInterval(int,ClientInfo)
+ int setViewDistance(int,ClientInfo)
+ String getMotd()
+ String setMotd(String,ClientInfo)
+ void <init>(DedicatedServer,JsonRpcLogger)
```
...
```diff
+ boolean clear(MinecraftApi,ClientInfo)
- List clear(MinecraftApi,ClientInfo)
```
...
- List lambda$registerDebugValues$5()
- List lambda$registerDebugValues$6(ServerLevel)
...
```diff
- BlockPos lambda$static$3(BoundingBox)
- BlockPos lambda$static$4(BoundingBox)
- BoundingBox lambda$static$5(BlockPos,BlockPos)
```
...
- Node[] lambda$findPath$4(int)
+ Path lambda$findPath$1(Map,Target)
- Path lambda$findPath$3(Map,Target)
+ Target lambda$findPath$0(BlockPos)
- Target lambda$findPath$1(BlockPos)
- void setCaptureDebug(BooleanSupplier)
```
...
```diff
- void setDebugListener(Consumer)
```
...
```diff
- void forEachUpdatedPos(Consumer)
```
...
```diff
+ void lambda$causeNeighborUpdates$1(List,BlockPos,Integer)
