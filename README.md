...
```diff
+ RealmsServer lambda$onInvite$5(long,String)
...
```diff
- void lambda$renderArmorPiece$0(ArmorItem,PoseStack,MultiBufferSource,int,boolean,HumanoidModel,boolean,ArmorTrim)
+ void lambda$renderArmorPiece$0(ArmorItem,PoseStack,MultiBufferSource,int,HumanoidModel,boolean,ArmorTrim)
+ void renderGlint(PoseStack,MultiBufferSource,int,HumanoidModel)
...
- void renderTrim(ArmorMaterial,PoseStack,MultiBufferSource,int,ArmorTrim,boolean,HumanoidModel,boolean,float,float,float)
...
- void colorBlockWithDye(Consumer,List,List)
```
...
```diff
+ void assertRedstoneSignal(BlockPos,Direction,IntPredicate,Supplier)
```
...
```diff
+ void checkSupportingBlock(boolean,Vec3)
...
+ int getEntityCount(Level,AABB,Class)
```
