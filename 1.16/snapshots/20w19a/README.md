...
# Comparison with <a href="https://github.com/PixiGeko/Minecraft-generated-data/tree/20w18a">20w18a</a>
## File structure
...
```diff
+ blocks/prevent_mob_spawning_inside.json
```
...
+ HashMap distCount
+ String lastDistData
+ Pair lambda$invalidateAndUpdate$3(BiomeSource,int,int,int[])
...
<details><summary>net.minecraft.util.thread.ProcessorHandle</summary>

```diff
+ CompletableFuture askEither(Function)
+ void lambda$askEither$0(Either)
```
...
```diff
- ImmutableMap POSE_DISMOUNT_HEIGHTS
...
```diff
+ Tag$Named PREVENT_MOB_SPAWNING_INSIDE
```
...
```diff
+ Vec3 getDismountLocationInDirection(LivingEntity)
```
...
<details><summary>net.minecraft.world.level.block.RedStoneWireBlock$1</summary>
