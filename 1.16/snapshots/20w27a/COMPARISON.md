## Comparison with [1.16.1](https://github.com/PixiGeko/Minecraft-generated-data/tree/1.16.1)
...
<br/>
<table><tr><th></th><th align="left">1.16.1</th><th>20w27a</th></tr><tr><td>World version</td><td><pre>2567</pre></td><td><pre>2569</pre></td></tr><tr><td>Protocol version</td><td><pre>736</pre></td><td><pre>738</pre></td></tr></table>
<h3>Libraries<a name="version-data-libraries"></a></h3>
<details>
<summary>
Versions
</summary>
<table><tr><th></th><th align="left">1.16.1</th><th>20w27a</th></tr><tr><td>org.lwjgl:lwjgl-glfw</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-glfw</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-jemalloc</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-jemalloc</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-openal</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-openal</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-opengl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-opengl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-stb</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-stb</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-tinyfd</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl-tinyfd</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr><tr><td>org.lwjgl:lwjgl</td><td><pre>3.2.1</pre></td><td><pre>3.2.2</pre></td></tr></table>
</details>
</details>
<hr/>
...
```
XXX.arguments.blocks.BlockPredicateArgument +2M -2M
```
```
XXX.minecraft.core.BlockSourceImpl +2M -2M | +1P -1P
```
```
XXX.server.players.GameProfileCache$GameProfileInfo +5M -3M | +1P -1P
```
```
XXX.world.entity.AgableMob +1M -1M | +1P -1P
```
```
XXX.world.entity.EntityDimensions +1M
```
```
...
```
```
XXX.ai.behavior.StrollToPoi +1M -1M | +1P
```
```
XXX.ai.behavior.WorkAtComposter +1M
```
```
XXX.ai.memory.MemoryModuleType +2P -3P
```
```
XXX.monster.piglin.Piglin +3M -8M
```
```
XXX.entity.npc.CatSpawner +2M -2M
```
```
XXX.entity.npc.Villager +7M -10M
```
```
...
```
```
XXX.world.inventory.RecipeBookMenu +1P
```
```
...
+ XXX.client.resources.ResourcePack
...
```
XXX.blaze3d.pipeline.RenderTarget +2M
```
```
net.minecraft.Util +2M
```
```
XXX.minecraft.client.ClientRecipeBook +9M -8M | +1P -2P
```
```
XXX.minecraft.client.Minecraft +1M -1M
```
```
XXX.screens.recipebook.AbstractFurnaceRecipeBookComponent -3M | -4P
```
```
XXX.screens.recipebook.RecipeBookComponent +1M -1M
```
```
XXX.screens.recipebook.RecipeCollection +2M -2M
```
```
XXX.screens.recipebook.SmokingRecipeBookComponent -4M
```
```
XXX.client.multiplayer.ClientPacketListener +1M -1M | +1P -1P
```
```
XXX.client.renderer.PostPass +1M -1M
```
```
XXX.renderer.texture.AbstractTexture +1M
```
```
XXX.renderer.texture.DynamicTexture +1M | +1P
```
```
...
```
```
XXX.server.players.GameProfileCache +9M -6M | +1P -3P
```
```
XXX.minecraft.sounds.SoundEvents +7P
```
```
XXX.minecraft.tags.BlockTags +1M -3M | +1P
```
```
XXX.minecraft.tags.FluidTags -3M
```
```
XXX.minecraft.tags.SerializationTags +3M -7M | +1P -5P
```
```
XXX.minecraft.tags.StaticTagHelper +6M -7M | +1P -1P
```
```
XXX.minecraft.tags.TagCollection +4M -13M | +3P -8P
```
```
XXX.ai.sensing.SensorType +2P -1P
```
```
XXX.entity.animal.Bee +2M -2M
```
```
...
```
```
XXX.entity.animal.MushroomCow +4M -4M
```
```
XXX.entity.animal.Ocelot +3M -3M
```
```
...
```
```
XXX.entity.animal.Turtle +3M -3M
```
```
XXX.animal.horse.AbstractHorse +2M -2M
```
```
XXX.animal.horse.Horse +2M -2M
```
```
XXX.animal.horse.Llama +3M -3M
```
```
...
```
```
XXX.world.item.FishBucketItem +1M -1M
```
```
...
```
```
XXX.level.block.InfestedBlock +2M -2M
```
```
XXX.level.block.OreBlock +1M -1M
```
```
XXX.level.block.RedStoneOreBlock +1M -1M
```
```
XXX.block.entity.TheEndGatewayBlockEntity +1M
```
```
...
```
```
XXX.levelgen.feature.BambooFeature +2M -2M
```
```
XXX.levelgen.feature.BasaltPillarFeature +2M -2M
```
```
XXX.levelgen.feature.BlockBlobFeature +2M -2M
```
```
XXX.levelgen.feature.BlueIceFeature +2M -2M
```
```
...
```
```
XXX.levelgen.feature.DeltaFeature +2M -2M
```
```
XXX.levelgen.feature.DesertWellFeature +2M -2M
```
```
XXX.levelgen.feature.EndGatewayFeature +2M -2M
```
```
XXX.levelgen.feature.EndPodiumFeature +2M -2M
```
```
XXX.levelgen.feature.FillLayerFeature +2M -2M
```
```
XXX.levelgen.feature.GlowstoneFeature +2M -2M
```
```
XXX.levelgen.feature.IceSpikeFeature +2M -2M
```
```
XXX.levelgen.feature.KelpFeature +2M -2M
```
```
XXX.levelgen.feature.NetherForestVegetationFeature +2M -2M
```
```
XXX.levelgen.feature.NoSurfaceOreFeature +2M -2M
```
```
XXX.levelgen.feature.RandomPatchFeature +2M -2M
```
```
...
```
```
XXX.levelgen.feature.SeagrassFeature +2M -2M
```
```
XXX.levelgen.feature.SimpleRandomSelectorFeature +2M -2M
```
```
XXX.levelgen.feature.SnowAndFreezeFeature +2M -2M
```
```
XXX.levelgen.feature.SpikeFeature +3M -3M
```
```
XXX.levelgen.feature.SpringFeature +2M -2M
```
```
XXX.levelgen.feature.TwistingVinesFeature +2M -2M
```
```
XXX.levelgen.feature.VoidStartPlatformFeature +2M -2M
```
```
XXX.levelgen.feature.WeightedConfiguredFeature +1M -1M
```
```
XXX.feature.treedecorators.AlterGroundDecorator +2M -2M
```
```
XXX.feature.treedecorators.CocoaDecorator +2M -2M
```
```
XXX.feature.treedecorators.TreeDecorator +1P -1P
```
```
XXX.feature.treedecorators.TrunkVineDecorator +2M -2M
```
```
XXX.levelgen.placement.FeatureDecorator +2M -2M
```
```
XXX.levelgen.structure.IglooPieces$IglooPiece +1M -1M
```
```
XXX.levelgen.structure.MineShaftPieces$MineShaftCrossing +1M -1M
```
```
XXX.levelgen.structure.NetherBridgePieces$NetherBridgePiece +1M -5M
```
```
XXX.levelgen.structure.NetherFossilPieces$NetherFossilPiece +1M -1M
```
```
XXX.levelgen.structure.OceanMonumentPieces$MonumentBuilding +7M -7M
```
```
XXX.levelgen.structure.OceanMonumentPieces$OceanMonumentPiece +4M -4M
```
```
...
```
```
XXX.levelgen.structure.StructurePiece +15M -15M
```
```
XXX.levelgen.structure.TemplateStructurePiece +1P -1P
```
