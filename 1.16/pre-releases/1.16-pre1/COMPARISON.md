## Comparison with [20w22a](https://github.com/PixiGeko/Minecraft-generated-data/tree/20w22a)
...
+ options.graphics.fabulous.tooltip: "Fabulous" graphics enables screen shaders to draw translucent objects per-pixel.
...
<br/>
<table>
<tr><th>Name</th><th>20w22a</th><th>1.16-pre1</th></tr>
<tr><th align="left"><div style="width:290px">subtitles.item.axe.strip</div></th><td>Debarking log</td><td>Axe scrapes</td></tr>
<tr><th align="left"><div style="width:290px">debug.gamemodes.press_f4</div></th><td>F4</td><td>[ F4 ]</td></tr>
<tr><th align="left"><div style="width:290px">debug.gamemodes.select_next</div></th><td>Next</td><td>%s Next</td></tr>
<tr><th align="left"><div style="width:290px">advancements.nether.obtain_crying_obsidian.title</div></th><td>Who Is Cutting Onions?</td><td>Who is Cutting Onions?</td></tr>
<tr><th align="left"><div style="width:290px">commands.locatebiome.notFound</div></th><td>Could not find a biome of type %s within reasonable distance</td><td>Could not find a biome of type "%s" within reasonable distance</td></tr>
<tr><th align="left"><div style="width:290px">commands.locatebiome.invalid</div></th><td>There is no biome named %s</td><td>There is no biome with type "%s"</td></tr>
</table>
<br/>
</details>
</details>
...
```
XXX.minecraft.commands.Commands +1M -1M
```
```
XXX.minecraft.core.SerializableUUID +3M -7M | -1P
```
```
XXX.data.recipes.RecipeProvider +1M
```
```
XXX.packs.repository.PackRepository +3M -2M | +1P -1P
```
```
XXX.world.entity.Entity +9M -7M
```
```
...
```
```
XXX.entity.animal.Bee +5M -5M | +3P -2P
```
```
...
```
```
XXX.entity.animal.IronGolem +6M -1M | +3P
```
```
XXX.entity.animal.MushroomCow +2M -2M
```
```
...
```
```
XXX.animal.horse.AbstractHorse +1P
```
```
...
```
```
XXX.item.crafting.RecipeType +1P
```
```
...
```
```
XXX.level.levelgen.WorldGenSettings +9M -5M | +1P -2P
```
```
XXX.level.pathfinder.BlockPathTypes +1P
```
```
XXX.level.pathfinder.WalkNodeEvaluator +3M -1M
```
```
XXX.level.storage.DerivedLevelData +1M -1M | -1P
```
```
XXX.level.storage.LevelStorageSource$LevelStorageAccess +8M -8M
```
...
- XXX.gui.screens.ResourcePackSelectScreen
...
```
XXX.gui.screens.VideoSettingsScreen +1P
```
```
...
```
```
XXX.minecraft.core.RegistryAccess +1P
```
```
XXX.minecraft.server.Main +2M -1M
```
```
XXX.minecraft.server.MinecraftServer +15M -14M
```
```
XXX.minecraft.server.ServerFunctionManager +1M
```
```
XXX.server.commands.DataPackCommand$Inserter +1P -1P
```
```
XXX.server.commands.SeedCommand +2M -2M
```
```
XXX.server.level.ServerPlayer +7M -4M | -1P
```
```
XXX.packs.repository.FolderRepositorySource +4M -4M | +1P
```
```
XXX.packs.repository.ServerPacksSource +2M -2M | +1P -1P
```
```
XXX.packs.resources.ResourceManagerReloadListener +1M -1M
```
```
XXX.packs.resources.SimpleReloadableResourceManager +4M -1M
```
```
XXX.packs.resources.SimpleReloadableResourceManager$ResourcePackLoadingFailure +2M -2M | +1P -1P
```
```
XXX.minecraft.util.DirectoryLock +1M | +1P
```
```
XXX.ai.behavior.GoToPotentialJobSite +7M -2M
```
```
XXX.ai.gossip.GossipContainer$GossipEntry -1M | +1P -1P
```
```
XXX.ai.memory.MemoryModuleType +3P -1P
```
```
XXX.world.level.Explosion +2M -1M | +1P
```
```
XXX.block.entity.BeehiveBlockEntity +1M
```
```
XXX.level.dimension.DimensionType +4M -6M | +4P -1P
```
